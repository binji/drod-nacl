// $Id: ListBoxWidget.cpp 9994 2012-03-24 17:24:59Z mrimer $
	...
	const bool bGrayLast)	//(in) if true, grayed items always go after non-gray items [default=false]
{
	...
	if (this->bSortAlphabetically)
	{
		...
		{
			if (bGrayLast && !bGrayed && (*iter)->bGrayed)
			...
			if (WCSicmp((*iter)->text.c_str(),pNew->text.c_str()) >= 0)
...
//Returns: Whether an item with this text exists.
{
	...
	{
		const WCHAR line_wc = towlower(this->Items[wLineNo]->text[0]);
		if (line_wc == wc)
			break; //found one
		if (this->bSortAlphabetically &&
								...
								//    the widget's rect.
{
	...
	//PaintChildren();
	ASSERT(this->Children.size() == 0); //list box doesn't have children
	...
	switch (KeyboardEvent.keysym.sym)
	{
		...
		default:
			if (this->bHotkeyItemSelection)
			{
				...
				{
					const WCHAR wc = KeyboardEvent.keysym.unicode ?
							W_t(KeyboardEvent.keysym.unicode + 96) : //convert Ctrl-<char> to <char> (lowercase)
							W_t(KeyboardEvent.keysym.sym);
...
//Params:
	const SDL_MouseButtonEvent &MouseButtonEvent)   //(in) Event to handle.
{
	CScrollableWidget::HandleMouseWheel(MouseButtonEvent);
