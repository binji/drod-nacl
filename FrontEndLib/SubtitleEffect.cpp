// $Id: SubtitleEffect.cpp 9322 2008-10-30 01:57:17Z mrimer $
...
//Params:
	...
	, pCoord(pCoord)
	, pSubtitles(NULL)
	, wXOffset(CBitmapManager::CX_TILE), wYOffset(CBitmapManager::CY_TILE) //position to lower-right of coord
...
//Set widget dimensions and prepare surface to render.
{
	...
	this->pOwnerWidget->DrawFilledRect(BorderRect, BGColor, this->pTextSurface);
	this->pOwnerWidget->DrawRect(BorderRect, Border, this->pTextSurface);

	//Draw text.
	UINT wLineH, wLineW;
	SDL_Rect TextRect = {wBorder, wBorder, this->w-widthBuffer, this->h-widthBuffer};
#ifdef RUSSIAN_BUILD
...
//Sets opacity of effect.
{
	...
	if (opacity < 255 || this->pTextSurface->format->Amask)
		SDL_SetAlpha(this->pTextSurface, SDL_SRCALPHA, this->opacity);
	else
		SDL_SetAlpha(this->pTextSurface, 0, 0);	//don't need any alpha on this surface
}

//*****************************************************************************
void CSubtitleEffect::SetOffset(const UINT wX, const UINT wY)
...
//*****************************************************************************
void CSubtitleEffect::AddTextLine(const WCHAR* pwczSetText, const Uint32 dwDuration, const SDL_Color& color) //[default=Black]
...
bool CSubtitleEffect::Draw(SDL_Surface* pDestSurface)
{
	...
	if (this->dwDuration && dwTimeSinceLastText > this->dwDuration + dwWaitForNewText)
	{
		...
		if (g_pTheBM->bAlpha)
		{
			...
			SDL_SetAlpha(this->pTextSurface, SDL_SRCALPHA, opacity);
		}
	}
...
void CSubtitleEffect::SetLocation()
{
	...
	const UINT wCX = this->wXOffset +
			(this->pCoord ? this->pCoord->wX * CBitmapManager::CX_TILE : 0);
	const UINT wCY = this->wYOffset +
			(this->pCoord ? this->pCoord->wY * CBitmapManager::CY_TILE : 0);
	const int wNewX = OwnerRect.x + wCX;
	const int wNewY = OwnerRect.y + wCY;
...
//	ASSERT(wCX - this->wXOffset < OwnerRect.w);
//	ASSERT(wCY - this->wYOffset < OwnerRect.h);
