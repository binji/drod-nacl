// $Id: EventHandlerWidget.cpp 9996 2012-03-24 17:28:21Z mrimer $
...
//The method exits when Deactivate() is called.
{
	...
	while (!this->bDeactivate)
	{
		...
		bool bHasFocus = (state & SDL_APPINPUTFOCUS) == SDL_APPINPUTFOCUS;
#if defined __APPLE__
		// The current SDL seems to sometimes report full-screen applications as not having
		// keyboard focus.  In order to cope with this:
		if (GetDestSurface()->flags & SDL_FULLSCREEN) bHasFocus = true;
#endif
...
void CEventHandlerWidget::ClearEventBuffer()
{
	...
	do {
		...
		if (SDL_PeepEvents(&event, 1, SDL_PEEKEVENT, SDL_QUITMASK))
			break;
		if (SDL_PeepEvents(&event, 1, SDL_PEEKEVENT, SDL_ACTIVEEVENTMASK))
			break;
	...
	if (m_RepeatingKey.keysym.sym != SDLK_UNKNOWN)
	{
		...
		if (!keyState[m_RepeatingKey.keysym.sym])
			m_RepeatingKey.keysym.sym = SDLK_UNKNOWN;
	}
}
...
{
	static Uint8 state = SDL_APPACTIVE & SDL_APPINPUTFOCUS;
	state = SDL_GetAppState();
#ifdef __APPLE__
	// a hack for OS X.  (SDL_GetAppState seems to not report that a full-screen app has keyboard focus.)
	if (GetDestSurface()->flags & SDL_FULLSCREEN) state = state | SDL_APPINPUTFOCUS;
#endif
	...
	//Clean up stray signals when app gains/loses focus, etc.
	ClearEvents();
}
...
//Handle things that do not occur in response to an SDL event.
{
	...
	if (bActive || bHasFocus)
	{
				...
				MOUSEDOWN_REPEAT_INITIAL_DELAY : MOUSEDOWN_REPEAT_CONTINUE_DELAY) )
		{
			...
			if (this->pHeldDownWidget->ContainsCoords(nX, nY))
			{
				...
				static SDL_MouseMotionEvent Motion = {SDL_MOUSEMOTION,
						0, 0, 0, 0, 0, 0};
				Motion.x = nX;
				Motion.y = nY;
		...
		dwNow - this->dwLastOnBetweenEventsCall > this->dwBetweenEventsInterval)
	{
		...
		{
			int nX, nY;
			Uint8 mb = SDL_GetMouseState(&nX, &nY);
			SDL_MouseMotionEvent nomotion = { SDL_MOUSEMOTION, 0, mb, nX, nY, 0, 0 };
...
//Simulate a mouse click on the widget.
{
	...
	static SDL_MouseButtonEvent Button = {SDL_MOUSEBUTTONDOWN, 0, 0, 0, 0, 0};
