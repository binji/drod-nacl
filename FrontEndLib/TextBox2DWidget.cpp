// $Id: TextBox2DWidget.cpp 9996 2012-03-24 17:28:21Z mrimer $
	...
	const SDL_KeyboardEvent &KeyboardEvent)   //(in) Event to handle.
{
	...
	switch (key)
	{
		...
		case SDLK_KP_ENTER:
		{
			...
			{
				const WCHAR wc = We(KeyboardEvent.keysym.unicode);
				TypeCharacter(wc);
			}
			break;
		}
...
//Returns: Whether or not the operation was successful (possible)
{
	...
	const UINT wLineOfTextH = g_pTheFM->GetFontLineHeight(this->fontType);
	const UINT wWindowHeight = this->h;
	const UINT wPageLines = wWindowHeight / wLineOfTextH;
	...
	int xPos, int yPos)  //(in) cursor's new position (in pixels)
{
	...
	if ((xPos <= 0 && yPos <= 0) || wLength == 0)
	{
		...
		while (wFirstPos < wLastPos)
		{
			...
			GetCursorPosition(this->wTextDisplayIndex, wIndex, wWidth, wHeight);
		...
		GetCursorPosition(this->wTextDisplayIndex, this->wCursorIndex, wWidth, wHeight);
	}
...
*/
		...
		GetCursorPosition(startIndex, wNewIndex, wCursorX, wCursorY);
		ASSERT(!wCursorY || (wCursorY == wLineOfTextH && !wCursorX)); //index should be on the first line following the index we provide
	...
	GetCursorPosition(startIndex, wNewIndex, this->wCursorX, this->wCursorY);
	ASSERT(!wCursorY || (wCursorY == wLineOfTextH && !wCursorX));
	wCursorY = lineY;
...
//Scroll view down the specified number of lines of text.
{
	...
	const int linesAboveEnd = this->lineIndices.size()+1 -
			(getLineContainingIndex(this->wTextDisplayIndex) + wDisplayH/lineH);
	const UINT lineDelta = linesAboveEnd < int(wLines) ? linesAboveEnd : wLines;
	if (lineDelta)
	{
		MoveViewDown(lineDelta);
		CalcAreas();
		GetCursorPosition(this->wTextDisplayIndex, this->wCursorIndex, this->wCursorX, this->wCursorY);
	}
}
...
//Scroll view down one page of text, if possible.
{
	const UINT wDisplayH = this->h;
	const UINT lineH = g_pTheFM->GetFontLineHeight(this->fontType);
	ScrollDownOneLine(wDisplayH/lineH);
}
...
//Scroll view up one page of text, if possible.
{
	const UINT wDisplayH = this->h;
	const UINT lineH = g_pTheFM->GetFontLineHeight(this->fontType);
	ScrollUpOneLine(wDisplayH/lineH);
}
...
//Maintains bookkeeping vars to optimize internal handling of the widget.
{
	...
	while (index < textSize)
	{
		...
		while (wFirstPos < wLastPos)
		{
			...
			GetCursorPosition(indexOfFirstCharOnLine, index, wCursorX, wCursorY);
	...
	const int nOffsetX, const int nOffsetY)   //(in) Drawing offsets.
{
	...
	if (nStartY == nEndY)
	{
		...
		g_pTheBM->Invert(GetDestSurface(), wX + nStartX, wY + nStartY,
				nEndX - nStartX, g_pTheFM->GetFontLineHeight(this->fontType)-1);
	} else {
