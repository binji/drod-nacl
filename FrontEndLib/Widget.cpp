// $Id: Widget.cpp 10037 2012-03-30 13:51:47Z mrimer $
	...
	const bool bUpdate)	//(in) [default=false]
{
			...
			iSeek != this->Children.end(); ++iSeek)
	{
		...
		SDL_Rect dirty = {pWidget->x + nOffsetX, pWidget->y + nOffsetY, pWidget->w, pWidget->h};
		SDL_Rect srcRect = {rect.x + dirty.x, rect.y + dirty.y, dirty.w, dirty.h};
...
{
	WCHAR wc = W_t(keysym.unicode);
	if (keysym.sym != SDLK_SLASH && wc == SDLK_QUESTION)
	{
		//Unicode value was not recognized properly,
		//hence SDL set the unicode value to '?'.
...
//*****************************************************************************
void CWidget::TranslateUnicodeKeysym(WCHAR& wc, const SDLKey sym, const bool bCaps)
{
	...
	switch (Language::GetLanguage())
	{
		...
		case Language::English:
			//no special translation supported (or needed) for this language-keyboard
		break;
	...
	const SDL_keysym& keysym) //(in)
{
	...
	if (!this->hotkeys.empty())
	{
		...
		WCHAR wc = TranslateUnicodeKeysym(keysym, false);
		wc = towlower(wc);
		UINT wIndex;
		for (wIndex = this->hotkeys.size(); wIndex--; )
			if (this->hotkeys[wIndex].key == (SDLKey)WCv(wc))
				return this->hotkeys[wIndex].tag;

		if (wc > 255) //Unicode hotkeys might have not been successfully
