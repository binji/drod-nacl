// $Id: Sound.cpp 9676 2011-09-05 21:36:47Z mrimer $
	...
	if (bMute)
	{
		if (musicVolume == -1 && soundVolume == -1 && voiceVolume == -1)
		{
			...
			if (this->nCurrentPlayingSongChannel >= 0)
				FSOUND_SetPaused(this->nCurrentPlayingSongChannel, true);
			//Pause sounds.
			for (UINT nChannelNo = SONG_CHANNEL_COUNT;
					nChannelNo < CHANNEL_COUNT; ++nChannelNo)
				FSOUND_SetPaused(nChannelNo, true);
#endif
		}
...
#else
			if (this->nCurrentPlayingSongChannel >= 0)
				FSOUND_SetPaused(this->nCurrentPlayingSongChannel, false);
			//Unpause sounds.
			for (UINT nChannelNo = SONG_CHANNEL_COUNT;
					nChannelNo < CHANNEL_COUNT; ++nChannelNo)
				FSOUND_SetPaused(nChannelNo, false);
#endif
...
const
{
	...
	if (nChannel != SOUNDLIB::SEID_NONE)
	{
		...
		return IsSoundPlayingOnChannel(nChannel);
	}
	...
	while (true) //non-looping.
	{
		...
		else
		{
			...
			if (FSOUND_Init(44100, SAMPLE_CHANNEL_COUNT, 0))
			{
				...
				CFiles Files;
				Files.AppendErrorLog("Not enough available channels for music." NEWLINE);
