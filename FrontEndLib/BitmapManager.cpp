 // $Id: BitmapManager.cpp 10030 2012-03-29 07:00:37Z mrimer $
	...
	const Uint8 nOpacity)   //(in)   Tile opacity (0 = transparent, 255 = opaque).
{
	if (nOpacity<255)
	{
		SDL_SetAlpha(pSrcSurface,SDL_SRCALPHA,nOpacity);
		SDL_BlitSurface(pSrcSurface, src, pDestSurface, dest);
		SDL_SetAlpha(pSrcSurface,0,0);
	} else {
		SDL_BlitSurface(pSrcSurface, src, pDestSurface, dest);
	}
}
	...
	const Uint8 nOpacity)   //(in)   Tile opacity (0 = transparent, 255 = opaque).
{
	...
	//Set coords and size.
	SDL_Rect src = {xSource,ySource,cxSize,cySize};
	SDL_Rect dest = {x,y,cxSize,cySize};
...
//
//Params:
	UINT* pwShadowTIs, const UINT wNumShadows,   //(in)   Tile shadow mask(s)
		...
		if (wNumShadows == 1)
		{
			...
			while (pDest != pEndOfRow)
			{
				...
				switch (*pSingleSrc) //examine only first color value
				{
					...
					{
						const float fFactor = 1.0f - ((255 - *pSingleSrc) * fShadowConvFactor);
						pDest[0] = static_cast<Uint8>(pDest[0] * fFactor);
						pDest[1] = static_cast<Uint8>(pDest[1] * fFactor);
						pDest[2] = static_cast<Uint8>(pDest[2] * fFactor);
					}
					break;
				}
				pDest += wBPP;
			...
			while (pDest != pEndOfRow)
			{
				...
				if (sum >= 255)  //Full shadow
				{
					...
					const float fFactor = 1.0f - (sum * fShadowConvFactor);
					pDest[0] = static_cast<Uint8>(pDest[0] * fFactor);
					pDest[1] = static_cast<Uint8>(pDest[1] * fFactor);
					pDest[2] = static_cast<Uint8>(pDest[2] * fFactor);
				}
...
//Params:
	...
	const Uint8 opacity)
{
	ASSERT(src.w == CX_TILE);
	ASSERT(src.h == CY_TILE);
	ASSERT(dest.w == CX_TILE);
	ASSERT(dest.h == CY_TILE);
	...
	const Uint8 opacity)
{
	...
	ASSERT(dest.w == CX_TILE);
	ASSERT(dest.h == CY_TILE);
	ASSERT(mask.w == CX_TILE);
	ASSERT(mask.h == CY_TILE);
	ASSERT(opacity > 0);	//wasteful for complete transparency
	...
	SDL_Surface *pDestSurface)    //(in)   Surface to blit to.
{
	...
	while (pSeek != pStop)
	{
		...
		while (pSeek != pEndOfRow)
		{
			...
			pSeek[0] = pSeek[1] = pSeek[2] = nValue;
			pSeek += wBPP;
		}
		pSeek += dwRowOffset;
	}
	...
	const UINT W, const UINT H) //surface width and height
{
	...
	if (dest.x < 0)
	{
		...
		if (src.w < wDelta) //rect is outside view area -- shrink to nothing
		{
	...
	if (dest.y < 0)
	{
		...
		if (src.h < wDelta) //a rect is outside view area -- shrink to nothing
		{
			src.w = src.h = dest.w = dest.h = 0;
			return;
		}
...
//Convert surface format to make blits fast (according to graphics hardware).
{
	...
	{
		pRet = SDL_DisplayFormatAlpha(pSurface);
	} else {
		pRet = SDL_DisplayFormat(pSurface);
	}
	...
	const int width, const int height)
{
	...
	if (pSrcSurface->format->BytesPerPixel==1 && pSrcSurface->format->palette)
		SDL_SetColors(pDestSurface, pSrcSurface->format->palette->colors, 0,
				pSrcSurface->format->palette->ncolors);
	if (pSrcSurface->flags & SDL_SRCCOLORKEY)
		SDL_SetColorKey(pDestSurface,
				(pSrcSurface->flags&SDL_RLEACCEL)|SDL_SRCCOLORKEY,
				pSrcSurface->format->colorkey);
	...
	SDL_Surface *pDestSurface)    //(in)   Surface to blit to.
{
	...
	else if (fLightPercent == 0.5f)
	{
		...
		while (pSeek != pStop)
		{
			...
			{
				//Set pixel intensity to % of original.
				pSeek[0] = (unsigned char)(fLightPercent * pSeek[0]);
				pSeek[1] = (unsigned char)(fLightPercent * pSeek[1]);
				pSeek[2] = (unsigned char)(fLightPercent * pSeek[2]);
				pSeek += wBPP;
			}
			pSeek += dwRowOffset;
		}
	...
	SDL_Surface *pDestSurface, const float fLightPercent)
{
	...
	while (pSeek != pStop)
	{
		...
		while (pSeek != pEndOfRow)
		{
			...
			{
				//Darken pixel.
				pSeek[0] = static_cast<Uint8>(pSeek[0] * fLightPercent);
				pSeek[1] = static_cast<Uint8>(pSeek[1] * fLightPercent);
				pSeek[2] = static_cast<Uint8>(pSeek[2] * fLightPercent);
			}
			pSeek += wBPP;
			pMask += wMaskBPP;
		}
...
//Set pixel intensities to faction of original, using indicated surface as a mask.
//When bUseColorKeyMask is set, non-transparent keyed pixels in the mask indicate
		...
		while (pSeek != pStop)
		{
			...
			while (pSeek != pEndOfRow)
			{
				if (pMask[0] != TransColor[0] || pMask[1] != TransColor[1] || pMask[2] != TransColor[2])
				{
					...
					pSeek[0] = static_cast<Uint8>(pSeek[0] * fLightFactor);
					pSeek[1] = static_cast<Uint8>(pSeek[1] * fLightFactor);
					pSeek[2] = static_cast<Uint8>(pSeek[2] * fLightFactor);
				}
...
{
	return !SDL_SetGamma(fGamma, fGamma, fGamma);
}
...
//Enables or disables a surface's transparent color key.
{
	...
	if (this->TileImageTypes[wTileImageNo] == TIT_Transparent)
	{
		if (!this->bIsColorKeySet[wSurfaceIndex])
		{
			...
			SDL_SetColorKey(pTileSurface, SDL_SRCCOLORKEY, TranspColor);
	...
	{
		SDL_SetColorKey(pTileSurface, 0, 0);
...
//Params:
	...
	UINT* pwShadowTIs, const UINT wNumShadows)
{
	ASSERT(pSrcSurface);
	ASSERT(pDestSurface);
		...
		switch (wNumShadows)
		{
			case 0:	//No shadow masks.
				while (pDest != pEndOfRow)
				{
					...
					//It is not assumed that a substantial amount of this image has
					//values of 0 or 255, so we won't waste CPU checking for them.
					fPercent = 1.0f - ((255 - pSrc[0]) * fShadowConvFactor); //examine only first color value
					pDest[0] = static_cast<Uint8>(pDest[0] * fPercent);
					pDest[1] = static_cast<Uint8>(pDest[1] * fPercent);
					pDest[2] = static_cast<Uint8>(pDest[2] * fPercent);
			...
			case 1:
				...
				while (pDest != pEndOfRow)
				{
					...
					if (val > 0)
					{
						const float fFactor = 1.0f - (val * fShadowConvFactor);
						pDest[0] = static_cast<Uint8>(pDest[0] * fFactor);
						pDest[1] = static_cast<Uint8>(pDest[1] * fFactor);
						pDest[2] = static_cast<Uint8>(pDest[2] * fFactor);
					}
	...
	const double sinAngle, const double cosAngle)
{
	...
	const int xd = ((srcWidth - pDestSurface->w) << 15);
	const int yd = ((srcHeight - pDestSurface->h) << 15);
	...
	{
		case 1:
			for (y = 0; y < pDestSurface->h; ++y)
			{
				Uint8 *pDstPos = (Uint8*)pDstRow;
				dx = (ax + (iSin * (cy - y))) + xd;
				dy = (ay - (iCos * (cy - y))) + yd;
				for (x = 0; x < pDestSurface->w; x++)
				{
	...
	const float fAngle)       //degrees
{
	...
	sx = sinAngle*x;
	sy = sinAngle*y;
   int nxmax = (int)(max(max(max(fabs(cx+sy), fabs(cx-sy)), fabs(-cx+sy)), fabs(-cx-sy)));
	int nymax = (int)(max(max(max(fabs(sx+cy), fabs(sx-cy)), fabs(-sx+cy)), fabs(-sx-cy)));
	...
	if (pSrcSurface->flags & SDL_SRCCOLORKEY)
	{
		bgcolor = pSrcSurface->format->colorkey;
	} else {
...
//Updates all screen regions that were marked as damaged.
//The surface passed in should be pointing to the screen surface.
//Call once per frame.
//NOTE: SDL does not optimize the case of overlapping rectangles.
//      We could add rectangle merging here if it was needed.
{
	if (!this->rects.size())
		return;

	ASSERT(pScreenSurface);
	ASSERT(!pScreenSurface->locked); //never call SDL_UpdateRect(s) when surface is locked
   if (this->bAlwaysUpdateScreen || this->rects.size() > 50) {
	   // If there are too many dirty rects, just redraw the entire screen - it will be faster.
	   SDL_UpdateRect(pScreenSurface, 0, 0, 0, 0);
   } else {
	   SDL_UpdateRects(pScreenSurface, this->rects.size(), &(*this->rects.begin()));
   }
	this->rects.clear();
}
