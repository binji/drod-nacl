// $Id: FontManager.cpp 10019 2012-03-26 14:36:51Z mrimer $
...
const
{
	...
	while (*pwczSeek != '\0' && (!wWidth || xDraw < wWidth))
	{
		...
		if (opacity < 255)
			SDL_SetAlpha(pText, SDL_SRCALPHA, opacity);
...
const
{
	...
	for (UINT wCharI = 0; wCharI < wStrLen; ++wCharI)
	{
		...
		if (width > 0)
		{
			SDL_Rect src = {0, 0, width, pText->h};
			SDL_Rect dest = {xDraw, yDraw, width, pText->h};
			if (opacity < 255) SDL_SetAlpha(pText, SDL_SRCALPHA, opacity);
			SDL_BlitSurface(pText, &src, pSurface, &dest);
			xDraw += pText->w;
		}
...
const
{
	...
	const bool draw_outline = pFont->bOutline && !bRenderFast;
	if (draw_outline)
	{
