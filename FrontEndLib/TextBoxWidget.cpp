// $Id: TextBoxWidget.cpp 9252 2008-09-26 01:14:01Z mrimer $
	...
	const SDL_KeyboardEvent &KeyboardEvent)   //(in) Event to handle.
{
   ...
   RequestPaint();
	...
	default:
		...
		if (KeyboardEvent.keysym.mod & KMOD_CTRL)
#else
		if ((KeyboardEvent.keysym.mod & KMOD_CTRL) || (KeyboardEvent.keysym.mod & KMOD_META))
#endif
		{
			switch (key)
			{
				...
				case SDLK_z:   //Undo
					...
					if ( (KeyboardEvent.keysym.mod & KMOD_META) && (KeyboardEvent.keysym.mod & KMOD_SHIFT) )
					{
						redo();
						break;
					}
			...
			WCHAR wc = We(KeyboardEvent.keysym.unicode);

			//Only printable characters or Unicode characters.
			if ((wc < ' ' || wc > '~') && wc < 256)
				return;

			wc = TranslateUnicodeKeysym(KeyboardEvent.keysym);
