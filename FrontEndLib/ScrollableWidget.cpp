// $Id: ScrollableWidget.cpp 8262 2007-09-25 13:30:34Z mrimer $
...
const
{
	...
	{
		rect.h -= CY_LEFT;
		if (wDesiredH > rect.h && !bVerticalScrollBar)
			rect.w -= CX_UP;
	}
}
...
{
	ASSERT(this->h > CY_UP + CY_DOWN + CY_INSET_BORDER * 2);
	ASSERT(this->w > CX_UP + CX_INSET_BORDER * 2);
	...
	else
	{
		...
		if (this->VScrollRect.h < CY_VPOS)  //No.
			CLEAR_RECT(this->VPosRect);
		else                    //Yes.
		{
			...
			if (this->VPosRect.h < CY_VPOS) this->VPosRect.h = CY_VPOS;
	...
	else
	{
		...
		if (this->HScrollRect.w < CX_HPOS)  //No.
			CLEAR_RECT(this->HPosRect);
		else                    //Yes.
		{
			...
			if (this->HPosRect.w < CX_HPOS) this->HPosRect.w = CX_HPOS;
...
{
	//Draw left button.
	SDL_Rect src = { (this->bIsLeftButtonPressed) ? X_LEFT_D : X_LEFT,
