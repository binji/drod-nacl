// $Id: TabbedMenuWidget.cpp 10035 2012-03-29 07:45:21Z mrimer $
										...
										//    the widget's rect.
{
	...
	//Draw menu area.
	//Fill menu surface.
	SDL_Rect rect = {this->x, this->y + this->tabHeight, this->w,
...
//Paint optional background image on the uniform color background.
{
	...
	{
		SDL_Rect dest = rect;
		SDL_SetAlpha(this->images[0], SDL_SRCALPHA, this->BGimageOpacity);
		SDL_BlitSurface(this->images[0], &rect, pDestSurface, &dest);
		SDL_SetAlpha(this->images[0], 0, 0);
	}
}
...
{
	//Select a tab.
	if (Button.y <= this->y + this->tabHeight)
	{
...
//Draw tabs on top.
{
	...
	for (UINT wTab=0; wTab<this->wNumTabs; ++wTab)
	{
		...
		SDL_Rect rect = {xPos, this->y, width, wTabHeight};
		SDL_FillRect(pDestSurface,&rect,BColor);
		...
		if (wTab == this->wSelectedTab)
		{
			...
			SDL_Rect rect = {xPos+1, this->y+wTabHeight-1, width-2, 2};
			PaintBGImage(rect, pDestSurface);
		}
		...
		if (this->tabTileNo[wTab] != NO_TAB_TILE)
		{
			...
			if (wFocusedTab == wTab)
			{
				...
				const SDL_Rect rect = {xPos + wXTile - BORDER, this->y + wYTile - BORDER,
			...
			{
				//Draw box inside tab.
				const SDL_Rect rect = {xPos + BORDER, this->y + BORDER,
