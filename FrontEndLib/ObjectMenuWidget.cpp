// $Id: ObjectMenuWidget.cpp 8745 2008-03-12 22:46:36Z mrimer $
...
//erase the dialog.
{
	...
	while (!bDeactivate && !pEventHandler->IsDeactivating())
	{
		...
		while (!bDeactivate && !pEventHandler->IsDeactivating() && SDL_PollEvent(&event)) 
		{
			switch (event.type)
			{
				case SDL_MOUSEBUTTONDOWN:
				{
					...
					if ((button.x < this->x) || (this->x + this->w <= button.x) ||
							(button.y < this->y) || (this->y + this->h <= button.y))
					{
						bDeactivate = true;
						break;
					}
