// $Id: Inset.cpp 8741 2008-03-12 22:44:33Z mrimer $
...
const Uint32 disabledColor = 190 << 16 | 181 << 8 | 165;   //medium brown
	...
	const bool bDisabled)         //(in) Gray out the center [default=false]
{
	...
	if (bUpdateRect)
	{
		ASSERT(!pDestSurface->locked);
		SDL_UpdateRect(pDestSurface, xLeftBevel, yTopBevel, wW, CY_INSET_BORDER);
		SDL_UpdateRect(pDestSurface, xLeftBevel, yBottomBevel, wW, CY_INSET_BORDER);
		SDL_UpdateRect(pDestSurface, xLeftBevel, yTopBevel+CY_INSET_BORDER, CX_INSET_BORDER, wH-CY_INSET_BORDER*2);
		SDL_UpdateRect(pDestSurface, xRightBevel, yTopBevel+CY_INSET_BORDER, CX_INSET_BORDER, wH-CY_INSET_BORDER*2);
	}
	...
	if (bDisabled)
	{
		...
		SDL_Rect center = {xCenter, yCenter, xRightBevel - xCenter, yBottomBevel - yCenter};
		SDL_FillRect(pDestSurface, &center, disabledColor);
		if (bUpdateRect)
			SDL_UpdateRect(pDestSurface, center.x, center.y, center.w, center.h);
	}
	else
		for (dest.y = yCenter; dest.y < yBottomBevel; dest.y += CY_CENTER)
		{
			...
			for (dest.x = xCenter; dest.x < xRightBevel; dest.x += CX_CENTER)
			{
				...
				SDL_BlitSurface(pPartsSurface, &src, pDestSurface, &dest);
				if (bUpdateRect)
					SDL_UpdateRect(pDestSurface, dest.x, dest.y, dest.w, dest.h);
			}
		}
}
