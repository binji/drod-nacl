// $Id: Wchar.cpp 9815 2012-02-04 12:21:20Z trick $
...
{
	return UTF8ToUCS2(psz, strlen(psz), wstr);
}
...
{
	WCHAR_t w = 0;
	str.reserve(str.length() + wstr.size());
	for (WSTRING::const_iterator i = wstr.begin(); i != wstr.end(); ++i)
	{
...
//*****************************************************************************
void UnicodeToUTF8(const WCHAR *pwsz, std::string &str)  // yay for naming
{
	BYTE* bytes = NULL;
...
{
	UnicodeToAscii(wcz, buffer);
	return isInteger(buffer);
}
...
WCHAR* _ltoW(long value, WCHAR* wcz, int radix)
{
	...
	do {
		int val = value % radix;
		if (val < 10)
			buffer[i++] = static_cast<char>((value % radix) + '0');
