// $Id: CoordSet.h 8019 2007-07-14 22:30:11Z trick $
...
{
public:
	typedef set<ROOMCOORD> CoordSet;
	...
	inline bool first(UINT &wX, UINT &wY) const
	{
		...
		if (iter == this->data.end()) return false;
	...
	{
		if (that.GetSize())
			for (UINT wY=that.GetRows(); wY--; )
				for (UINT wX=that.GetCols(); wX--; )
					if (that.GetAt(wX,wY))
						insert(wX,wY);
		return *this;
	}
