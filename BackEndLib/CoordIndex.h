// $Id: CoordIndex.h 8616 2008-01-31 20:27:15Z mrimer $
...
CCoordIndex_T<T>& CCoordIndex_T<T>::operator=(const CCoordIndex_T<T>& rhs)
{
	if (!rhs.wCols || !rhs.wRows)
	{
		...
		//works only for primitive types
		memcpy(this->pbytIndex, rhs.pbytIndex, rhs.wCols * rhs.wRows * sizeof(T));
...
//True if index was allocated, false if not.
{
	...
	//To make assertions fire in other methods that have been incorrectly 
	//called.
	...
	const T val)               //(in) default=1
{
	...
	if (this->pbytIndex[dwSquareI] == 0)
		++this->dwCoordCount;
	ASSERT(this->dwCoordCount <= this->wCols * this->wRows);
	this->pbytIndex[dwSquareI] = val;
}
...
{
	if (this->pbytIndex)
		memset(this->pbytIndex, val, this->wCols * this->wRows * sizeof(T));
	this->dwCoordCount = val ? this->wCols * this->wRows : 0;
}
...
{
	ASSERT(pCoords);
	for (UINT dwSquareI = this->wCols * this->wRows; dwSquareI--; )
	{
...
{
	ASSERT(oldVal != newVal); //wasteful when valled with same value

	const long inc = oldVal == 0 ? 1 : newVal == 0 ? -1 : 0;
