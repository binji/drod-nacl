// $Id: Internet.cpp 10018 2012-03-25 02:15:52Z mrimer $
...
curl_httppost* CInternet::pFormPostEnd = NULL;
...
int CInternet_HTTP_Get_Worker(void* pPtr)
{
	...
	curl_httppost* pPost = CInternet::pFormPostBegin;
//	curl_httppost* pEnd = CInternet::pFormPostEnd;

	CInternet::pFormPostBegin = NULL;
	CInternet::pFormPostEnd = NULL;
...
//Returns: Error code returned if handle response received (0 indicating success), else -1
{
	...
	return (int)found->second->eRetVal;
}
...
//True if the operation has been initialized.
{
   ...
   pInfo->pBuffer->Alloc(1000);
	...
	SDL_Thread* pThread = SDL_CreateThread(CInternet_HTTP_Get_Worker, pInfo);
...
{
	curl_formadd(&CInternet::pFormPostBegin, &CInternet::pFormPostEnd,
...
//Params:
	const string& strName,     //(in) Name
	const vector<string>& values)       //(in) Value
{
