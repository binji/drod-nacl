// $Id: Assert.h 8419 2007-11-30 22:02:45Z trick $
...
# else
#   define ASSERT(exp)          { if (!(exp)) AssertErr(__FILE__,__LINE__,#exp); } (void)0
#   define ASSERTP(exp, desc)   { if (!(exp)) AssertErr(__FILE__,__LINE__,(desc)); } (void)0
# endif
#else
#   define ASSERT(exp)
...
#ifdef _DEBUG
#  define VERIFY(exp) \
		{ if (!(exp)) AssertErr(__FILE__, __LINE__,#exp); } (void)0
#else
