// $Id: Files.cpp 10036 2012-03-30 12:53:57Z trick $
...
static const WCHAR wszOldHomeConfDir[] =
        {We('.'),We('c'),We('a'),We('r'),We('a'),We('v'),We('e'),We('l'),We(0)};
#else
...
//Returns: true if directory exists or was just created, false if not.
{
	...
	if (CFiles::WindowsCanBrowseUnicode())
		...
		string pzPath;
		UnicodeToAscii(pwzPath, pzPath);
		ret = _mkdir(pzPath.c_str());
	}
	if (ret == 0)
		return true;
	return errno == EEXIST;
	...
	const bool confirm_resource_file)
{
	...
	if (!binpath.empty())
		binpath += ":";
	UnicodeToCPath(CFiles::wszAppPath, binpath);
...
# ifdef USE_XDG_BASEDIR_SPEC
	...
	if (!GetNormalizedPathEnv("XDG_DATA_HOME", newconfpath))
	{
		...
		static const WCHAR localshare[] = { We('/'),We('.'),We('l'),We('o'),We('c'),We('a'),We('l'),We('/'),We('s'),We('h'),We('a'),We('r'),We('e'),We(0) };
		newconfpath = CFiles::wstrHomePath + localshare;
	}
...
//True if the directory was found, false if not.
{
	...
	if (!CFiles::WindowsCanBrowseUnicode()) {
		...
		UnicodeToAscii(wszFilepath, buffer);
