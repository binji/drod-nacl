// $Id: DbRooms.cpp 10072 2012-04-04 01:02:58Z TFMurphy $
...
//Serialize all texts in the room.
{
	...
	for (wIndex=0; wIndex<wSize; ++wIndex)
	{
		...
		strLocal += LONGTOSTR(pScroll->wX);
		PROPTAG(strLocal, P_Y);
		strLocal += LONGTOSTR(pScroll->wY);
	...
	const bool bRef)        //(in) Only export GUID reference (default = false)
{
	...
	if (bRef)
	{
		...
		str += LONGTOSTR((long)pRoom->bIsSecret);
		if (pRoom->dwDataID)
		{
			...
			if (g_pTheDB->Data.Exists(pRoom->dwDataID))
			{
				...
				str += LONGTOSTR(pRoom->dwDataID);
				str += PROPTAG(P_ImageStartX);
				str += LONGTOSTR(pRoom->wImageStartX);
				str += PROPTAG(P_ImageStartY);
				str += LONGTOSTR(pRoom->wImageStartY);
			}
		}
		...
		for (dwIndex=0; dwIndex<dwSize; ++dwIndex)
		{
			...
			for (dwAgentI=0; dwAgentI<dwNumAgents; ++dwAgentI)
			{
				...
				str += LONGTOSTR(pOrbAgent->action);
				str += PROPTAG(P_X);
		...
		for (dwIndex=0; dwIndex<dwSize; ++dwIndex)
		{
			...
			str += LONGTOSTR(pExit->dwEntranceID);
			str += PROPTAG(P_Left);
				...
				checkpoint != pRoom->checkpoints.end(); ++checkpoint)
		{
			...
			str += LONGTOSTR(checkpoint->wX);
			str += PROPTAG(P_Y);
			str += LONGTOSTR(checkpoint->wY);
			str += CLOSETAG;
		}
	...
	bool &bSaveRecord)      //(out) whether record should be saved
{
	...
	switch (pType)
	{
		...
		case P_LevelID:
			this->dwLevelID = static_cast<UINT>(atol(str));
			if (!this->dwLevelID)
				return MID_FileCorrupted;  //corrupt file
			...
			break;
		case P_RoomX:
			this->dwRoomX = static_cast<UINT>(atol(str));
			break;
		case P_RoomY:
			this->dwRoomY = static_cast<UINT>(atol(str));
		...
		case P_Style:  //deprecated since 3.0
		{
			...
			UINT wStyle = static_cast<UINT>(atoi(str));
			if (wStyle > 9) wStyle = 1;

			//Convert old style number to current style name.
			static const UINT wCurrentNumberOfStyles = 9;
			...
			static const UINT styleMapping[2][9] = {{1, 2, 4, 9, 3, 8, 5, 7, 6},{1, 2, 3, 9, 8, 4, 5, 7, 6}};
			const UINT wGameIndex = info.wVersion >= 200 ? 1 : 0;

			ASSERT(styleMapping[wGameIndex][wStyle-1] > 0);
			ASSERT(styleMapping[wGameIndex][wStyle-1] <= wCurrentNumberOfStyles);
			AsciiToUnicode(styleNames[styleMapping[wGameIndex][wStyle-1]-1], this->style);
		}
		break;
		case P_StyleName:
			ASSERT(info.wVersion >= 301);
		...
		case P_IsRequired:
			...
			break;
		case P_IsSecret:
			this->bIsSecret = (atoi(str) != 0);
			break;
		case P_DataID:
			this->dwDataID = static_cast<UINT>(atol(str));
			if (this->dwDataID)
			{
				//Set to local ID.
			...
			break;
		case P_ImageStartX:
			this->wImageStartX = static_cast<UINT>(atoi(str));
			break;
		case P_ImageStartY:
			this->wImageStartY = static_cast<UINT>(atoi(str));
			break;
		case P_Squares:
		{
		...
		case VP_OrbAgents:
			...
			switch (pType)
			{
				case P_Start:
					...
					break;
				case P_Type:
					pImportOrbAgent->action = static_cast<OrbAgentType>(atoi(str));
	      ...
	      ASSERT(pType == P_Start || pImportOrb);
			...
			switch (pType)
			{
				case P_Start:
					...
					break;
				case P_Type:
					pImportOrb->eType = static_cast<OrbType>(atoi(str));
		...
		case VP_Pieces:
			...
			switch (pType)
			{
				case P_Start:
					...
					break;
				case P_Type:
					pImportPiece->wTileNo = static_cast<UINT>(atoi(str));
					break;
				case P_X:
					pImportPiece->wX = static_cast<UINT>(atoi(str));
					...
					break;
				case P_Y:
					pImportPiece->wY = static_cast<UINT>(atoi(str));
		...
		case VP_Exits:
			...
			switch (pType)
			{
				case P_Start:
					...
					break;
				case P_EntranceID:
					pImportExit->dwEntranceID = static_cast<UINT>(atol(str));
					break;
				case P_Left:
		...
		case VP_Checkpoints:
			switch (pType)
			{
				case P_Start:
					break;
				case P_X:
					importCheckpoint.wX = static_cast<UINT>(atoi(str));
					if (importCheckpoint.wX >= this->wRoomCols)
						return MID_FileCorrupted;
					break;
				case P_Y:
					importCheckpoint.wY = static_cast<UINT>(atoi(str));
...
//Reflects everything in the room in the X direction.
{
	...
	for (wY=this->wRoomRows; wY--; )
	{
		...
		for (wX=this->wRoomCols/2; wX--; )
		{
			...
			if (wSquare2)
				this->coveredTSquares.Add(wX, wY, wSquare2);
			else this->coveredTSquares.Remove(wX, wY);
			if (wSquare)
				this->coveredTSquares.Add(wRefX, wY, wSquare);
			else this->coveredTSquares.Remove(wRefX, wY);
	...
	{
		COrbData *pOrb = *orb;
		pOrb->wX = this->wRoomCols-1 - pOrb->wX;
		//Reflect orb agents.
			...
			stair != this->Exits.end(); ++stair)
	{
		...
		pStair->wLeft = this->wRoomCols-1 - pStair->wRight;
		pStair->wRight = this->wRoomCols-1 - oldwLeft;
	}
...
//Reflects everything in the room in the Y direction.
{
	...
	for (wX=this->wRoomCols; wX--; )
	{
		...
		for (wY=this->wRoomRows/2; wY--; )
		{
			...
			if (wSquare)
				this->coveredTSquares.Add(wX, wRefY, wSquare);
			else this->coveredTSquares.Remove(wX, wRefY);
	...
	{
		COrbData *pOrb = *orb;
		pOrb->wY = (this->wRoomRows-1) - pOrb->wY;
		//Reflect orb agents.
		for (vector<COrbAgentData*>::const_iterator agent=pOrb->agents.begin();
				agent != pOrb->agents.end(); ++agent)
		{
			...
			stair != this->Exits.end(); ++stair)
	{
		...
		pStair->wTop = (this->wRoomRows-1) - pStair->wBottom;
		pStair->wBottom = (this->wRoomRows-1) - oldwTop;
	}
...
//Returns: True if successful, false if not.
{
	...
	CMonsterFactory mf;
	CMonster *pNew = mf.GetNewMonster(M_CHARACTER);
	ASSERT(pNew);
...
const
{
	...
	switch (wAppearance)
	{
		...
		case M_SPIDER:	case M_GOBLIN: case M_ROCKGOLEM:
		case M_TARBABY: case M_MUDBABY: case M_GELBABY:
		case M_AUMTLICH: case M_WUBBA: case M_FEGUNDO:
			if ( !(wTileNo == T_EMPTY || wTileNo==T_SCROLL ||
					wTileNo == T_FUSE || wTileNo == T_TOKEN))
			{
		...
		default:
			...
			if ( !(wTileNo == T_EMPTY || wTileNo==T_SCROLL || bIsPotion(wTileNo) ||
					wTileNo == T_FUSE || wTileNo == T_TOKEN))
				bObstacle = true;
		break;
	}
...
const
{
	...
	if (bIsTrapdoor(wTileNo) || !(bIsFloor(wTileNo) || bIsPlatform(wTileNo) ||
			(bIsShallowWater(wTileNo) &&
			(wDoubleType != M_CLONE || this->pCurrentGame->swordsman.CanWadeInShallowWater())) ||
	...
	if (this->pCurrentGame->swordsman.HasSword() &&
			this->pCurrentGame->swordsman.wSwordX == wX &&
...
//*****************************************************************************
void CDbRoom::KillGhostsOutsideWall(CCueEvents &CueEvents)
	...
	const CEntity* pKillingEntity) //(in) Optional killing Entity who caused death [default=NULL]
{
	...
	bool bRemoveMonster = mType != M_HALPH && mType != M_HALPH2 && mType != M_CLONE;
	if (bRemoveMonster && mType == M_CHARACTER)
	{
		CCharacter *pCharacter = DYN_CAST(CCharacter*, CMonster*, pMonster);
		bRemoveMonster = pCharacter->GetImperative() != ScriptFlag::MissionCritical ||
	...
	if (bRemoveMonster || bForce)
	{
		...
		if (pMonster->bProcessing)
			pMonster->bUnlink = true; //Defer unlinking until the monsters turn is over
		else
		...
		case M_CHARACTER:
		{
			...
			switch (pCharacter->GetImperative())
			{
				case ScriptFlag::MissionCritical:
					...
					{
						//If NPC was Halph, then flag that Halph died.
						const UINT identity = pCharacter->GetIdentity();
				...
				break;
				case ScriptFlag::RequiredToConquer:
					DecMonsterCount();
				break;
				default: break; //Don't decrement monster count.
			}
...
bool CDbRoom::GetNearestEntranceTo(const UINT wX, const UINT wY, const MovementType eMovement, UINT &wEX, UINT &wEY)
{
	...
	const bool bHasSword = player.HasSword();
	SORTPOINTS sp;
	...
	for (SORTPOINTS::const_iterator it = sp.begin(); it != sp.end(); ++it)
	{
		...
		if (GetMonsterAtSquare(p.wX, p.wY) != NULL || IsMonsterSwordAt(p.wX, p.wY)
				|| (bHasSword && player.wSwordX == p.wX && player.wSwordY == p.wY))
			continue;
		wEX = p.wX;
		wEY = p.wY;
		return true;
...
//Toggle token -- modify room elements/behavior.
{
	...
	switch (tType)
	{
		case SwordDisarm:
		{
			this->pCurrentGame->swordsman.bNoSword = !this->pCurrentGame->swordsman.bNoSword;
			...
			for (CMonster *pMonster = this->pFirstMonster; pMonster != NULL;
				...
				switch (pMonster->wType)
				{
					case M_DECOY: case M_CLONE: case M_MIMIC:
					{
						...
						pDouble->bNoSword = this->pCurrentGame->swordsman.bNoSword;
					}
					break;
					default: break;
				}
		...
		case PowerTarget:
			...
			this->pCurrentGame->swordsman.bIsTarget = true;
	...
	CCueEvents &CueEvents)     //(in/out)
{
			...
			fuse!=this->LitFuses.end(); ++fuse)
	{
		...
		switch (GetTSquare(wX,wY))
		{
			...
			case T_FLOW_EDGE: //can grow onto a burning fuse
				//do nothing
			break;
			default:
...
//Traverses room squares to change each arrow only once.
{
	...
	switch (tType)
	{
		...
		case ConquerToken:
			...
			for (wY=this->wRoomRows; wY--; )
				for (wX=this->wRoomCols; wX--; )
					if (GetBottomTSquare(wX,wY) == T_TOKEN)
					{
...
//Process any objects falling down at (x,y)
{
	...
	switch (wTSquare)
	{
		...
		break;
		case T_FLOW_SOURCE: case T_FLOW_INNER: case T_FLOW_EDGE:
	...
	{
		//What was under the mirror?
		wTSquare = GetTSquare(wX, wY);
		if (wTSquare == T_SCROLL || bIsPotion(wTSquare) || (wTSquare == T_FUSE && bTrapdoorFell))
			//A CID_ObjectFell was already created for the mirror, and the front end will animate that.  Just delete the second object.
			Plot(wX, wY, T_EMPTY);
	}
	...
	if (bIsShallowWater(wOSquare))
	{
		if (pMonster->wType == M_ROCKGOLEM && !pMonster->IsAlive())
		{
			Plot(wX, wY, T_STEP_STONE);
			CueEvents.Add(CID_Splash, new CCoord(pMonster->wX, pMonster->wY), true);
			KillMonster(pMonster, CueEvents);
			this->pCurrentGame->TallyKill(pMonster->wType);
			return;
		} else if (pMonster->CanWadeInShallowWater())
	...
	if (pMonster->IsLongMonster())
	{
		if (pMonster->wType == M_ROCKGIANT)
		{
			...
			KillMonster(pMonster, CueEvents);
			this->pCurrentGame->TallyKill(M_ROCKGIANT);  //counts as a kill
			...
			if (bIsDeepWater(wOTileAtHead))
			{
				...
				if (bSerpentDiedFromTruncation)
				{
					...
					KillMonster(pMonster, CueEvents);
					this->pCurrentGame->TallyKill(pMonster->wType);  //counts as a kill
				}
...
//A prioritized list of general room changes that are checked each game turn.
{
	...
	if (bFullMove)
	{
		...
		if (CueEvents.HasOccurred(CID_TarGrew) || CueEvents.HasOccurred(CID_MudGrew) || CueEvents.HasOccurred(CID_GelGrew))
		{
			...
			const bool bRoomHasTar = this->wTarLeft > 0;
	...
	KillGhostsOutsideWall(CueEvents);
	...
	const bool bKillsFegundo)     //[default=true]
{
	...
	UINT wTileNo = GetTSquare(wX,wY);
	// Destroy mirrors first, and then deal with anything that was under them.
	if (wTileNo == T_MIRROR)
	{
		Plot(wX,wY,T_EMPTY);
		wTileNo = GetTSquare(wX,wY);
	...
	switch (wTileNo)
	{
		...
		case T_FLOW_INNER: case T_FLOW_EDGE: //Briar tiles are destroyed.
	...
	if (pMonster && (bKillsFegundo || pMonster->wType != M_FEGUNDO))
	{
		...
		switch (pMonster->wType)
		{
			...
			{
				CCharacter *pCharacter = DYN_CAST(CCharacter*, CMonster*, pMonster);
				eImperative = pCharacter->GetImperative();
			}
			break;
			case M_FEGUNDOASHES:
	...
	CCueEvents& CueEvents) //(in/out)
{
	...
	//Check for a sword at destination location pointing right towards the
	//direction the mirror is being pushed from, the mirror will break.
	if (wTile == T_MIRROR && IsValidColRow(wDestX+dx, wDestY+dy))
	{
		CMonster *pMonster = GetMonsterAtSquare(wDestX+dx, wDestY+dy);
		if (pMonster && pMonster->HasSwordAt(wDestX, wDestY))
		{
			CueEvents.Add(CID_MirrorShattered, new CMoveCoord(wSrcX, wSrcY, nGetO(dx,dy)), true);
			return;
		}
		//Is the swordsman's sword in the square?
		if (this->pCurrentGame->IsPlayerAt(wDestX+dx, wDestY+dy))
		{
			if (this->pCurrentGame->swordsman.HasSword() &&
				this->pCurrentGame->swordsman.wSwordX == wDestX &&
				this->pCurrentGame->swordsman.wSwordY == wDestY)
			{
				CueEvents.Add(CID_MirrorShattered, new CMoveCoord(wSrcX, wSrcY, nGetO(dx,dy)), true);
				return;
			}
	...
	switch (GetOSquare(wDestX, wDestY))
	{
		...
		case T_SHALLOW_WATER:
			//Mirrors fall to create stepping stones
			if (wTile == T_MIRROR)
...
const
{
	...
	{
		case T_WALL_M:
			return this->pCurrentGame && this->pCurrentGame->bHoldMastered;
		case T_WALL: case T_WALL2: case T_WALL_IMAGE:
			return wAppearance == M_SEEP;
		case T_PIT: case T_PIT_IMAGE:
			return bIsEntityFlying(wAppearance);
		case T_WATER:
			return bIsEntityFlying(wAppearance) || bIsEntitySwimming(wAppearance);
		case T_SHALLOW_WATER:
			return bIsEntityFlying(wAppearance) || bIsEntitySwimming(wAppearance) ||
				this->pCurrentGame->swordsman.GetWaterTraversalState() >= WTrv_CanWade;
		...
		case T_STAIRS: case T_STAIRS_UP:
		case T_DOOR_YO: case T_DOOR_GO: case T_DOOR_CO: case T_DOOR_RO: case T_DOOR_BO:
		case T_STEP_STONE: case T_TRAPDOOR: case T_TRAPDOOR2: case T_PRESSPLATE:
		case T_BRIDGE: case T_BRIDGE_H: case T_BRIDGE_V:
...
const
{
	...
	UINT wTileNo = static_cast<unsigned char>(this->pszTSquares[ARRAYINDEX(wX,wY)]);
	if (wTileNo == T_MIRROR) {
		UINT wCoveredTile = this->coveredTSquares.GetAt(wX,wY);
		if (wCoveredTile != T_EMPTY)
			wTileNo = wCoveredTile;
	}
...
const
{
	...
	return (UINT) (this->pszTParams[ARRAYINDEX(wX,wY)]);
}
...
const
{
	...
	if (bMovementSupportsPartialObstacles(eMovement))
	{
		...
		switch (GetFSquare(wX, wY))
		{
			...
			case T_ARROW_N:  return DMASK_SE | DMASK_S | DMASK_SW;
			case T_ARROW_NE: return DMASK_S | DMASK_SW | DMASK_W;
			case T_ARROW_E:  return DMASK_SW | DMASK_W | DMASK_NW;
			case T_ARROW_SE: return DMASK_W | DMASK_NW | DMASK_N;
			case T_ARROW_S:  return DMASK_NW | DMASK_N | DMASK_NE;
			case T_ARROW_SW: return DMASK_N | DMASK_NE | DMASK_E;
			case T_ARROW_W:  return DMASK_NE | DMASK_E | DMASK_SE;
			case T_ARROW_NW: return DMASK_E | DMASK_SE | DMASK_S;
...
//Frees resources associated with this object and resets member vars.
{
	...
	this->bPersistentCitizenMovement = this->bHasConquerToken = false;
...
//Unpacks squares from database (version 1.6 - 3.0) into a format that the game will use.
//This routine will fail if the data is incompatible with the game engine.
//
//Params:
	const BYTE *pSrc, //(in) Buffer of tile data from database.
...
//True if successful, false if not.
{
	...
	ASSERT(!this->pszTParams);
	this->pszTParams = new BYTE[dwSquareCount];
	...
	if (version < 3)
	{
		...
		memset(this->pszTParams, 0, dwSquareCount * sizeof(BYTE));
		memset(this->pszFSquares, T_EMPTY, dwSquareCount * sizeof(char));

		return UnpackSquares1_6(pSrc, dwSrcSize, dwSquareCount, this->pszOSquares, this->pszFSquares, this->pszTSquares);
	}
	if (version > 5) return false; //DROD supports up to room data format version 5.

	char *pWriteO = this->pszOSquares, *pWriteF = this->pszFSquares, *pWriteT = this->pszTSquares;  //shorthand
	BYTE *pWriteTP = this->pszTParams;
	BYTE numTiles;
	char tileNo;
	BYTE param=0;
	...
	if (version < 5)
	{
		...
		for (UINT wIndex=0; wIndex<dwSquareCount; ++wIndex, ++pWriteT)
		{
			switch (*pWriteT)
			{
				...
				case T_NODIAGONAL:
					this->pszFSquares[wIndex] = *pWriteT;
					*pWriteT = T_EMPTY;
				break;
			}
		}
...
//Params:
	...
	const UINT dwSrcSize,  //(in) Size of buffer.
	const UINT dwSquareCount,//(in) Number of squares in room.
	char *pszOSquares,   //(in/out) Accepts buffer allocated to room dwSquareCount.
						//Returns buffer filled with opaque-layer squares.
	char *pszFSquares,   //(in/out) Buffer has explicit floor-layer tiles set
...
//True if successful, false if not.
{
	...
	while (pRead < pStopReading)
	{
		...
		*(pWriteO++) = (char)wOpaqueTileNo;
		...
		{
			pszFSquares[pWriteT - pszTSquares] = (char)wTransparentTileNo;
			*(pWriteT++) = (char)T_EMPTY;
		}
		else
...
//
//Params:
	const bool bRetainCharacters) //(in) if true, don't delete non-influential monsters
											//[default=false]
{
	...
	while (pSeek)
	{
		...
		if (bRetainCharacters && (pDelete->wType == M_WUBBA ||
				pDelete->wType == M_HALPH || pDelete->wType == M_HALPH2 ||
				pDelete->wType == M_SLAYER || pDelete->wType == M_SLAYER2 ||
				pDelete->wType == M_CHARACTER || pDelete->wType == M_CITIZEN ||
				bIsBeethroDouble(pDelete->wType) || pDelete->wType == M_FEGUNDO ||
				pDelete->wType == M_STALWART || pDelete->wType == M_STALWART2))
		{
	...
	UINT& wMonsterCountAtStart) //(in/out)
{
	...
	//Not conquered, but if no monsters or conquer tokens in it, then add to conquered list immediately.
	else if (!wMonsterCountAtStart && !this->bHasConquerToken)
	{
	...
	KillGhostsOutsideWall(CueEvents); //remove before any doors change
	...
	if (bIsCurrentLevelComplete)
	{
		...
		ToggleTiles(T_DOOR_C, T_DOOR_CO); //blue/exit door
	}
	...
	if (this->wTrapDoorsLeft == 0)
		ToggleTiles(T_DOOR_R, T_DOOR_RO); //red door
	...
	const UINT wTarType)    //(in)   Tar tile type
{
	...
	CCoordSet contiguousGel, newGel;
	const bool bGel = wTarType == T_GEL;
	bool bMotherIsAlive = false;
	...
	for (x = 0; x < this->wRoomCols; ++x)
		for (y = 0; y < this->wRoomRows; ++y)
		{
			...
			if ((bIsFloor(wTile) || bIsOpenDoor(wTile) || bIsPlatform(wTile)) &&
				 GetFSquare(x, y) == T_EMPTY && GetTSquare(x, y) == T_EMPTY &&
				!(x == wSManX && y == wSManY) &&
			        (!pMonster || pMonster->wType == wMotherType || babies.Exists(x,y)))
			{
				for (UINT o = 0; o < ORIENTATION_COUNT; ++o)
				{
					if (o == NO_ORIENTATION) continue;
	...
	const bool bDelayBabyMoves)   //(in)  Whether babies can move immediately when processed [default=false]
{
	...
	if (bShallowPathmapNeeded && !this->pPathMap[GROUND_AND_SHALLOW_WATER])
		CreatePathMaps();
			...
			if (pMonster)
			{
				...
				if (nType != -1)
				{
					...
					if (pMonster->wType == M_GELBABY && bIsShallowWater(GetOSquare(wX,wY)))
					{
						CueEvents.Add(CID_Splash, new CCoord(wX,wY), true);
						KillMonster(pMonster, CueEvents);
						//Tally kill as new type
						this->pCurrentGame->TallyKill(nType);
					} else {
...
//of them just moved to this square, else false.
{
	...
	if (player.HasSword())
	{
		swords.insert(player.wSwordX, player.wSwordY);

		//Check whether player is moving sword.
		if (player.wSwordMovement != NO_ORIENTATION)
			swordMoved.insert(player.wSwordX, player.wSwordY);
	}
...
const
{
	...
	if (this->pCurrentGame && this->pCurrentGame->swordsman.HasSword() && IsValidColRow(
			this->pCurrentGame->swordsman.wSwordX,	this->pCurrentGame->swordsman.wSwordY))
		SwordCoords.Add(this->pCurrentGame->swordsman.wSwordX,
				this->pCurrentGame->swordsman.wSwordY, 1+this->pCurrentGame->swordsman.wO); //avoid 0 values
}
		...
		switch (pAgent->action)
		{
			case OA_TOGGLE:
				if (bIsLight(wTTile))
					...
					ToggleYellowDoor(pAgent->wX, pAgent->wY);
			break;
			...
			case OA_CLOSE:
				if (bIsLight(wTTile))
					...
					CloseYellowDoor(pAgent->wX, pAgent->wY);
			break;
	...
	CCueEvents &CueEvents)     //(in/out)
{
	...
	ASSERT(bIsPit(newOTile) || bIsDeepWater(newOTile));
	Plot(wX, wY, newOTile);
...
//*****************************************************************************
void CDbRoom::RemoveStalwart(CMonster *pMonster)
//Remove stalwart from room stalwart list.
{
	list<CPlayerDouble*>::const_iterator iSeek;
	for (iSeek = this->stalwarts.begin(); iSeek != this->stalwarts.end(); ++iSeek)
	{
		if (*iSeek == pMonster)
		{
			//Found it.
			this->stalwarts.remove(*iSeek);
			return;
		}
	}
	ASSERT(iSeek != this->stalwarts.end());   //indicates stalwart wasn't stored in list (bad)
}
			...
			if (bThisTileChanged)
			{
				...
				if (bIsDoor(wTile))
				{
					switch (GetTSquare(wX, wY))
					{
						...
						break;
						case T_MIRROR:
							if (this->coveredTSquares.GetAt(wX,wY) == T_FUSE)
							{
	...
	bool bUnderObject)   //(in) default=false
{
	...
	switch (TILE_LAYER[wTileNo])
	{
		...
		case 1: //Transparent layer.
		{
			...
			bool bReplacedMirror = (oldTile == T_MIRROR && wTileNo != T_EMPTY);
			if (wTileNo == T_EMPTY && this->coveredTSquares.GetAt(wX,wY) != T_EMPTY)
			{
				this->pszTSquares[wSquareIndex] = static_cast<unsigned char>(
						this->coveredTSquares.GetAt(wX,wY));
				ASSERT(T_EMPTY == 0);
				this->coveredTSquares.Remove(wX,wY); //set to T_EMPTY
			} else if (bUnderObject && oldTile == T_MIRROR &&
				(wTileNo == T_SCROLL || wTileNo == T_FUSE || wTileNo == T_TOKEN))
			{
				bReplacedMirror = false;
				this->coveredTSquares.Add(wX,wY,wTileNo);
			}
			else
				this->pszTSquares[wSquareIndex] = static_cast<unsigned char>(wTileNo);
			//If a mirror was removed, also remove anything that was underneath
			if (bReplacedMirror && this->coveredTSquares.GetAt(wX,wY) != T_EMPTY)
			{
				this->coveredTSquares.Remove(wX,wY);
			}
...
void CDbRoom::SetTParam(const UINT wX, const UINT wY, const BYTE value)
{
	...
	UINT wTTile = GetBottomTSquare(wX,wY);
...
//Returns: pointer to record to be saved into database (must be deleted by caller).
{
	const UINT dwSquareCount = CalcRoomArea();
	ASSERT(dwSquareCount);
	char *pSquares = new char[dwSquareCount*5 + 1];  //max size possible
	...
	c4_View &MonstersView)     //(in) Open view to fill.
{
	...
	{
		pMonster = mf.GetNewMonster(M_HALPH);
	...
	{
		pMonster = mf.GetNewMonster(M_HALPH2);
		pMonster->wX = enters->wX;
		pMonster->wY = enters->wY;
		pMonster->Save(MonstersView[wCount++]);
		delete pMonster;
	...
	{
		pMonster = mf.GetNewMonster(M_SLAYER);
		pMonster->wX = enters->wX;
		pMonster->wY = enters->wY;
		pMonster->Save(MonstersView[wCount++]);
		delete pMonster;
	...
	{
		pMonster = mf.GetNewMonster(M_SLAYER2);
		pMonster->wX = enters->wX;
		pMonster->wY = enters->wY;
		pMonster->Save(MonstersView[wCount++]);
		delete pMonster;
...
//*****************************************************************************
CDbRoom* CDbRoom::MakeCopy(CImportInfo& info) const
//Replicates room data for new record entry in DB.
{
	...
	CDbData::CopyObject(info, pCopy->dwDataID);
	...
	const bool bCopyLocalInfo) //(in) default = true
{
	...
	memcpy(this->pszTParams, Src.pszTParams, dwSquareCount * sizeof(BYTE));
...
//of squares in the room.
{
	...
	this->bBetterVision = false;
	this->bPersistentCitizenMovement = this->bHasConquerToken = false;
	this->bCheckForHoldMastery = this->bIsSecret;
	this->briars.clear();
...
const
{
	...
	if (!tileMask.get(GetOSquare(wX, wY)) && !tileMask.get(GetTSquare(wX, wY))) return;
