// $Id: DbLevels.cpp 10060 2012-03-31 13:47:22Z mrimer $
	...
	const bool bRef)        //(in) Only export GUID reference (default = false)
{
	...
	if (!bRef)
	{
		...
		str += LONGTOSTR(pLevel->dwOrderIndex);
		...
		str += LONGTOSTR(long((time_t)pLevel->Created));
		str += PROPTAG(P_LastUpdated);
		str += LONGTOSTR(long((time_t)pLevel->LastUpdated));
		str += PROPTAG(P_IsRequired);
		str += LONGTOSTR(long(pLevel->bIsRequired));
	}
...
//saving it to the DB.
//
//Post-Cond: hold's Update method has been called
//
	...
	CImportInfo& info) //(in/out)	
{
	...
	const UINT dwEntranceRoomID = SaveCopyOfRooms(pNewLevel->dwLevelID, info);
	...
	const CDbHold::HoldStatus officialStatus = CDbHold::GetOfficialHoldStatus();
	const bool bTCBHold = (pOldHold->status == officialStatus && pNewHold->status == officialStatus);
	...
	for (UINT wIndex=0; wIndex<pOldHold->Entrances.size(); ++wIndex)
	{
		...
		if (pOldEntranceRoom->dwLevelID == this->dwLevelID)
		{
			...
			CEntranceData *pEntrance = new CEntranceData(*(pOldHold->Entrances[wIndex]));
			ASSERT(pEntrance);
	...
	//Update character script IDs in all copied rooms.
	CDb db;
	CIDSet roomIDs = CDb::getRoomsInLevel(pNewLevel->dwLevelID);
	for (CIDSet::const_iterator room = roomIDs.begin(); room != roomIDs.end(); ++room)
	{
		CDbRoom *pRoom = db.Rooms.GetByID(*room);
		ASSERT(pRoom);
		bool bScriptIDsUpdated = false;
		...
		while (pMonster)
		{
			if (pMonster->wType == M_CHARACTER)
			{
				...
				pCharacter->ChangeHold(pOldHold, pNewHold, info, !bTCBHold);
				if (bTCBHold)
				{
					//When copying levels to an alternate version of the TCB hold,
					...
					bScriptIDsUpdated = true;
				}
			}
			pMonster = pMonster->pNext;
		}
		if (bScriptIDsUpdated)
			pRoom->Update();
		delete pRoom;
	}
	...
	bool &bSaveRecord)      //(out) whether record should be saved
{
	...
	switch (pType)
	{
		...
		case P_GID_LevelIndex:
			this->dwLevelIndex = static_cast<UINT>(atol(str));
			if (!this->dwLevelIndex)
				return MID_FileCorrupted;  //corrupt file
			break;
		case P_OrderIndex:
			this->dwOrderIndex = static_cast<UINT>(atol(str));
			if (!this->dwOrderIndex)
				return MID_FileCorrupted;  //corrupt file
			break;
		case P_IsRequired:
			this->bIsRequired = (atol(str) != 0);
			break;
		...
		case P_RoomID:
			pEntrance->dwRoomID = static_cast<UINT>(atol(str));
			//Room hasn't been read in yet and local ID must be set later
			break;
		case P_X:
			pEntrance->wX = static_cast<UINT>(atoi(str));
			break;
		case P_Y:
			pEntrance->wY = static_cast<UINT>(atoi(str));
			break;
		case P_O:
			pEntrance->wO = static_cast<UINT>(atoi(str));
	...
	CImportInfo& info)
{
	...
	for (CDbRoom *pRoom = db.Rooms.GetFirst(); pRoom != NULL; pRoom = db.Rooms.GetNext())
	{
		...
		CDbRoom *pRoomCopy = pRoom->MakeCopy(info); //must make new message texts
		...
		ASSERT(info.RoomIDMap.find(pRoom->dwRoomID) == info.RoomIDMap.end());
