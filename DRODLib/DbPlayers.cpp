// $Id: DbPlayers.cpp 10060 2012-03-31 13:47:22Z mrimer $
...
//Simply serialize a player reference.
{
	...
	str += LONGTOSTR(long((time_t)pPlayer->Created));
	...
	const bool bRef)        //(in) Only export GUID reference (default = false)
{
	...
	if (bRef)
		...
		str += LONGTOSTR(long((time_t)pPlayer->LastUpdated));
	...
	if (bRef)
	{
		...
		} else if (dbRefs.eSaveType == ST_Progress || dbRefs.eSaveType == ST_ProgressEndHold) {
	...
	bool &bSaveRecord)      //(out) whether record should be saved
{
	...
	switch (pType)
	{
		...
		case P_Settings:
		{
			...
			{
				const bool bEyeCandy = this->Settings.GetVar("EyeCandy", true);
				this->Settings.SetVar("EyeCandy", BYTE(bEyeCandy ? Metadata::GetInt(MetaKey::MAX_EYE_CANDY) : 0));
			}
			delete[] data;
			break;
		}
