// $Id: DbXML.cpp 10060 2012-03-31 13:47:22Z mrimer $
	...
	void* /*userData*/, const char *name, const char **atts)
{
	...
	if (bLanguageMod)
	{
		...
		if (pType != P_Invalid)
		{
			//Object properties.
			CDbBase *pBase = dbRecordStack.back();
			MESSAGE_ID status = pBase->SetProperty(pType, atts);
...
//Ones that are invalid get deleted.
{
	...
	string str;
	if (CFiles::GetGameProfileString("Customizing", "ValidateSavesOnImport", str))
			...
			hold!=info.localHoldIDs.end(); ++hold)
	{
		...
		for (iter = savedGameIDs.begin(), wCount = 0; iter != savedGameIDs.end(); ++iter)
		{
			...
			const UINT dwRoomID = db.SavedGames.GetRoomIDofSavedGame(*iter);
			if (dwRoomID != 0)
			{
	...
	const CImportInfo::ImportType type)		//(in)	Type of data being imported
{
	...
	if (type == CImportInfo::LanguageMod)
	{
		...
		const UINT fileSize = buffer.Size();
		ASSERT(fileSize);
		decodedSize = static_cast<ULONG>(fileSize * 1.6);  //start with a reasonably-sized buffer
		decodedBuf = new BYTE[decodedSize+1]; //allow null-termination
		if (!decodedBuf) return MID_OutOfMemory;
		int res;
		do {
