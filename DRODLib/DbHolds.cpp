// $Id: DbHolds.cpp 10060 2012-03-31 13:47:22Z mrimer $
...
//Import utility vars.
CEntranceData *pImportEntrance = NULL;
	...
	const bool bCopyLocalInfo)  //[default=true]
{
	...
	this->bShowDescription = Src.bShowDescription;
...
//Serialize a hold reference, its texts, and the texts of every child object.
{
	...
	str += LONGTOSTR(long((time_t)pHold->Created));
	str += PROPTAG(P_GID_PlayerID);
	str += LONGTOSTR(pHold->dwPlayerID);
	str += PROPTAG(P_LastUpdated);
	str += LONGTOSTR(long((time_t)pHold->LastUpdated));  //timestamp provides hold "version" info
	...
	for (level = levels.begin(); level != levels.end(); ++level)
	{
		...
		for (UINT wIndex=pHold->Entrances.size(); wIndex--; )
		{
			...
			if (roomIDs.has(pEntrance->dwRoomID))
			{
				...
				str += LONGTOSTR(pEntrance->dwEntranceID);
				str += CLOSESTARTTAG;
	...
	const bool bRef)        //(in) Only export GUID reference (default = false)
{
	...
	str += LONGTOSTR((long)pHold->status);
	if (!bRef)
	{
		...
		str += LONGTOSTR(pHold->dwScriptID);
		str += PROPTAG(P_VarID);
		str += LONGTOSTR(pHold->dwVarID);
		str += PROPTAG(P_CharID);
		str += LONGTOSTR(pHold->dwCharID);
	...
	else
	{
		...
		for (dwIndex=0; dwIndex<dwSize; ++dwIndex)
		{
			...
			str += LONGTOSTR(pEntrance->dwRoomID);
			str += PROPTAG(P_X);
			str += LONGTOSTR(pEntrance->wX);
			str += PROPTAG(P_Y);
			str += LONGTOSTR(pEntrance->wY);
			str += PROPTAG(P_O);
			str += LONGTOSTR(pEntrance->wO);
			str += PROPTAG(P_IsMainEntrance);
			str += LONGTOSTR(long(pEntrance->bIsMainEntrance));
			str += PROPTAG(P_ShowDescription);
			str += LONGTOSTR(long(pEntrance->bShowDescription));
			str += CLOSETAG;
		}
...
const
{
	...
	if (savedGameID)
	{
		...
		if (roomsExplored == numSecretRooms)
			return numSecretRooms; //all rooms -- don't need to check any more saved games
	}
				...
				for (UINT i=0; i<pCharacter->commands.size(); ++i)
				{
					...
					switch (c.command)
					{
						...
						case CCharacterCommand::CC_WaitForVar:
						{
							...
							{
								const WSTRING wstr = ScriptVars::getVarNameW(ScriptVars::Predefined(c.x));
								pVarName = wstr.c_str();
							} else {
								pVarName = pHold->GetVarName(c.x);
							}

							AddScriptVarRef(varMap, pVarName, roomID, pRoom->wRoomCols,
...
{
	//Copy custom character media.
	for (UINT wIndex=pNewHold->characters.size(); wIndex--; )
	{
		HoldCharacter& ch = pNewHold->characters[wIndex];

		CDbData::CopyObject(info, ch.dwDataID_Avatar);
		CDbData::CopyObject(info, ch.dwDataID_Tiles);
	}
}
...
//The caller must remember to Update() the hold if this character gets used.
{
	...
	this->characters.push_back(HoldCharacter(dwNewCharID, pwszName, dwNewScriptID));
	return dwNewCharID;
}
...
//*****************************************************************************
HoldCharacter* CDbHold::GetCharacter(const UINT dwCharID)
...
//Returns: character ID for first record found with given name, if exists, else 0
{
			...
			character != this->characters.end(); ++character)
	{
		...
		if (!WCScmp(c.charNameText.c_str(), pwszName))
			return c.dwCharID;
	}
	...
	c4_View &CharactersView)    //(in) Open view containing 0 or more characters
{
	...
	for (UINT wCharI=0; wCharI<wSize; ++wCharI)
	{
		...
		this->characters.push_back(HoldCharacter(
				(UINT)(p_CharID(row)),
				name.c_str(),
				dwScriptID,
				(UINT)(p_Type(row)),
...
//Level exits from all rooms in the level are reset.
//If dwLevelSupplantedID was the ID of the hold's first level, the new
//level becomes the hold's first level.
//
//NOTE: Following this call, all existing room objects from this level must be discarded,
//otherwise Updating them will revert their level exit destinations to the old values.
//
//Params:
	CDbLevel *pLevel,                //(in) Level being inserted
	const UINT dwLevelSupplantedID) //(in) Level this one is placed in front of [default=0]
{
	ASSERT(IsOpen());
	...
	//Reset level exits.
	//Each iteration processes a room in the level being inserted.
	c4_View RoomsView;
	CIDSet roomsInLevel = CDb::getRoomsInLevel(pLevel->dwLevelID);
			...
			room != roomsInLevel.end(); ++room)
	{
		...
		//Exits in rooms of the inserted level are reset.
		c4_View ExitsView = p_Exits(row);
		const UINT dwExitCount = ExitsView.GetSize();
		for (UINT wExitI = 0; wExitI < dwExitCount; ++wExitI)
			p_EntranceID(ExitsView[wExitI]) = 0;

		//Update exits in character scripts.
		CDbRoom *pRoom = g_pTheDB->Rooms.GetByID(*room);
		ASSERT(pRoom);
		bool bUpdated = false;
		CMonster *pMonster = pRoom->pFirstMonster;
...
//Returns: pointer to new hold
{
	...
	//Since entrance IDs are reordered in SaveCopyOfLevels, the current entrance
	//data must be saved.
	ENTRANCE_VECTOR newEntrances = pNewHold->Entrances;
	pNewHold->Entrances.clear();
	...
	//Revert to saved copy of entrances.
	pNewHold->ClearEntrances();
	pNewHold->Entrances = newEntrances;

	//Update all room IDs in new hold's entrance list.
	UINT wIndex;
	for (wIndex=pNewHold->Entrances.size(); wIndex--; )
	{
			...
			setIter != savedGamesInHold.end(); ++setIter)
	{
		...
		ASSERT(pSavedGame);
		if (pSavedGame->eType == ST_RoomBegin || pSavedGame->eType == ST_Checkpoint ||
			pSavedGame->eType == ST_LevelBegin || pSavedGame->eType == ST_Continue ||
			pSavedGame->eType == ST_EndHold)
		{
			...
			{
				localID = info.RoomIDMap.find(pSavedGame->dwRoomID);
			...
			{
				localID = info.RoomIDMap.find(*room);
				ASSERT(localID != info.RoomIDMap.end());
				ASSERT(localID->second != 0);
...
//
//Params:
	CDbHold *pHold,  //(in) hold new levels belong to
	CImportInfo& info) //(in/out)
{
	...
	for (CIDSet::const_iterator level = levelsInHold.begin(); level != levelsInHold.end(); ++level)
	{
		...
		CDbLevel *pNewLevel = pLevel->MakeCopy(pHold, info);
		pNewLevel->dwHoldID = pHold->dwHoldID;
		pNewLevel->Update();
	...
	pHold->dwLevelID = dwEntranceLevelID;
	pHold->Update();
	return true;
}
	...
	const bool bCopyLocalInfo) //(in) default = true
{
	...
	{
		HoldCharacter c(*chIter, !bCopyLocalInfo);
	...
	switch (pType)
	{
		...
		case P_DescriptionMessage:
			if (pImportEntrance)
			{
				...
				CEntranceData *pEntrance = GetEntrance(pImportEntrance->dwEntranceID);
				if (pEntrance)
	...
	bool &bSaveRecord)      //(out) whether record should be saved
{
	...
	switch (pType)
	{
		case P_HoldID:
		{
			this->dwHoldID = static_cast<UINT>(atol(str));
			if (!this->dwHoldID)
		...
		case P_GID_NewLevelIndex:
			this->dwNewLevelIndex = static_cast<UINT>(atol(str));
		...
		case P_ScriptID:
			this->dwScriptID = static_cast<UINT>(atol(str));
			break;
		case P_Status:
			this->status = static_cast<HoldStatus>(atol(str));
			break;
		case P_VarID:
			this->dwVarID = static_cast<UINT>(atol(str));
			break;
		case P_CharID:
			this->dwCharID = static_cast<UINT>(atol(str));
			break;
		default:
			return MID_FileCorrupted;
	}
...
{
	static HoldVar importVar;
	static HoldCharacter importChar;
	...
	switch (vpType)
	{
		case VP_Entrances:
			...
			switch (pType)
			{
				case P_Start:
					ASSERT(!pImportEntrance);
					pImportEntrance = new CEntranceData();
					break;
				case P_EntranceID:
					pImportEntrance->dwEntranceID = static_cast<UINT>(atol(str));
					break;
				case P_DescriptionMessage:
				{
					WSTRING data;
					Base64::decode(str,data);
					pImportEntrance->DescriptionText = data.c_str();
					break;
				}
				case P_RoomID:
					pImportEntrance->dwRoomID = static_cast<UINT>(atol(str));
					//Room hasn't been read in yet and local ID must be set later
					break;
				case P_X:
					pImportEntrance->wX = static_cast<UINT>(atoi(str));
					break;
				case P_Y:
					pImportEntrance->wY = static_cast<UINT>(atoi(str));
					break;
				case P_O:
					pImportEntrance->wO = static_cast<UINT>(atoi(str));
					break;
				case P_IsMainEntrance:
					pImportEntrance->bIsMainEntrance = (atoi(str) != 0);
					break;
				case P_ShowDescription:
					pImportEntrance->bShowDescription = (atoi(str) != 0);
					break;
				case P_End:
					//Finish processing
					if (info.typeBeingImported == CImportInfo::LanguageMod)
						delete pImportEntrance; //don't re-add existing hold members
					else
						this->Entrances.push_back(pImportEntrance);
		...
		case VP_Vars:
			switch (pType)
			{
				case P_Start:
					...
					ASSERT(!importVar.dwVarID);
					ASSERT(importVar.varNameText.empty());
					ASSERT(info.typeBeingImported != CImportInfo::LanguageMod);
					break;
				case P_VarID:
					importVar.dwVarID = static_cast<UINT>(atol(str));
					break;
				case P_VarNameText:
				{
					WSTRING text;
					Base64::decode(str,text);
					importVar.varNameText = text.c_str();
					break;
				}
				case P_End:
					//Finish processing
					this->vars.push_back(importVar);
					importVar.dwVarID = 0;
					importVar.varNameText.resize(0);
					break;
				default:
					importVar.dwVarID = 0;
					importVar.varNameText.resize(0);
		...
		case VP_Characters:
			switch (pType)
			{
				case P_Start:
					//Start with fresh record.
					ASSERT(!importChar.dwCharID);
					ASSERT(importChar.charNameText.empty());
					ASSERT(info.typeBeingImported != CImportInfo::LanguageMod);
					break;
				case P_CharID:
					importChar.dwCharID = static_cast<UINT>(atol(str));
					break;
				case P_CharNameText:
				{
					WSTRING text;
					Base64::decode(str,text);
					importChar.charNameText = text.c_str();
					break;
				}
				case P_Type:
					importChar.wType = static_cast<UINT>(atol(str));
					break;
				case P_DataID:
					importChar.dwDataID_Avatar = static_cast<UINT>(atol(str));
					if (importChar.dwDataID_Avatar)
					{
						//Set to local ID.
						PrimaryKeyMap::iterator localID = info.DataIDMap.find(importChar.dwDataID_Avatar);
						if (localID == info.DataIDMap.end())
							return MID_FileCorrupted;  //record should have been loaded already
						importChar.dwDataID_Avatar = localID->second;
					}
					break;
				case P_DataIDTiles:
					importChar.dwDataID_Tiles = static_cast<UINT>(atol(str));
					if (importChar.dwDataID_Tiles)
					{
						//Set to local ID.
						PrimaryKeyMap::iterator localID = info.DataIDMap.find(importChar.dwDataID_Tiles);
						if (localID == info.DataIDMap.end())
							return MID_FileCorrupted;  //record should have been loaded already
						importChar.dwDataID_Tiles = localID->second;
					}
					break;
				case P_ExtraVars:
				{
					...
					importChar.ExtraVars = pCharacter->ExtraVars;
					UINT dwScriptID = importChar.ExtraVars.GetVar(scriptIDstr, UINT(0));
					importChar.dwScriptID = dwScriptID;
					delete pCharacter;
				...
				case P_End:
					//Finish processing
					this->characters.push_back(importChar);
					importChar.dwCharID = importChar.dwDataID_Avatar = importChar.dwDataID_Tiles = 0;
					importChar.wType = 0;
					importChar.charNameText.resize(0);
					importChar.dwScriptID = 0;
					importChar.ExtraVars.Clear();
					break;
				default:
					importChar.dwCharID = importChar.dwDataID_Avatar = importChar.dwDataID_Tiles = 0;
					importChar.wType = 0;
	...
	c4_View &EntrancesView)    //(in) Open view to fill.
{
	...
	for (wEntranceI=0; wEntranceI < wSize; ++wEntranceI)
	{
		...
		p_ShowDescription(row) = entrance.bShowDescription;
	}
