// $Id: AntHill.cpp 9742 2011-10-22 16:12:27Z mrimer $
							...
							//sound or graphical effects.
{
	...
	if (this->pCurrentGame->wSpawnCycleCount % ANT_SPAWN_SPEED == 0)
		for (int y = -1; y <= 1; ++y)
			for (int x = -1; x <= 1; ++x)
			{
				...
				if (!this->pCurrentGame->pRoom->IsValidColRow(ex, ey)) continue;

				const UINT wOSquare = this->pCurrentGame->pRoom->GetOSquare(ex, ey);
				const UINT wTSquare = this->pCurrentGame->pRoom->GetTSquare(ex, ey);
				if (
					//Is o-square open?
					bIsWater(wOSquare) &&
					//Not player
	...
	if (fBestDistance < INFINITY)
	{
		...
		CMonster *m = this->pCurrentGame->pRoom->AddNewMonster(M_WATERSKIPPER, wBestX, wBestY);
		m->bIsFirstTurn = true;
