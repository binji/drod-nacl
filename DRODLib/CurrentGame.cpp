// $Id: CurrentGame.cpp 10073 2012-04-04 03:04:29Z TFMurphy $
	...
	const UINT identity, const UINT wX, const UINT wY, const UINT wO)
{
	...
	if (IsValidMonsterType(identity))
	{
		...
		if (identity != M_SEEP && this->pRoom->GetOSquare(wX, wY) == T_PRESSPLATE
					&& !bIsEntityFlying(identity))
...
{
	return this->swordsman.st.getVar(ScriptVars::Predefined(varIndex));
}
	...
	UINT newVal)
{
	...
	this->swordsman.st.setVar(ScriptVars::Predefined(itemID), newVal);
}
...
//Show the current game show a level entrance.
{
	...
	return pEntrance && pEntrance->bShowDescription;
}
...
//Returns: whether player is at (x,y)
{
	...
	return wX == this->swordsman.wX && wY == this->swordsman.wY;
}
...
{
	if (!this->swordsman.HasSword())
		return false;

	return wX == this->swordsman.wSwordX && wY == this->swordsman.wSwordY;
}
...
//True if successful, false if not.
{
	...
	this->wStartRoomAppearance = M_BEETHRO; //start as Beethro by default
	this->bStartRoomSwordOff = false;       //start with sword by default
	this->wStartRoomWaterTraversal = WTrv_AsPlayerRole;   //start with default wading properties
	...
	CCueEvents &CueEvents)
{
	...
	this->bStartRoomSwordOff = this->swordsman.bSwordOff;
...
//Params:
	...
	const UINT wAppearance,	//player starting appearance
	const bool bSwordOff,   //whether player has sword or not
	const UINT wWaterTraversal, //whether player has overridden shallow water traversal
...
//True if successful, false if not.
{
	...
	this->wStartRoomX = wX;
	this->wStartRoomY = wY;
	this->wStartRoomAppearance = wAppearance;
	this->bStartRoomSwordOff = bSwordOff;
	this->wStartRoomWaterTraversal = wWaterTraversal;
	this->swordsman.wX = this->swordsman.wPrevX = CDbSavedGame::wStartRoomX;
	this->swordsman.wY = this->swordsman.wPrevY = CDbSavedGame::wStartRoomY;
	this->swordsman.SetOrientation(CDbSavedGame::wStartRoomO);
	SetRoomStartToPlayer();
...
//True if successful, false if not.
{
	...
	this->swordsman.bSwordOff = CDbSavedGame::bStartRoomSwordOff;
...
//unexpected state, false if not.
{
	...
	for (comIter = this->Commands.Get(wCommandI);
	     comIter != this->Commands.end();
	     comIter = this->Commands.GetNext(), ++wCommandI)
	{
		...
		if (this->swordsman.wPlacingDoubleType && nCommand != CMD_DOUBLE && !bIsAnswerCommand(nCommand))
		{
			...
			FreezeCommands();
			comIter = this->Commands.GetCurrent();
	...
	const bool bProcessedAll = (comIter == this->Commands.end());
...
//Call after processing a character's turn.
{
	...
	if (pCharacter->bReplaced)
	{
		...
		if (pMonster && CueEvents.HasOccurredWith(CID_FegundoToAsh, pMonster))
	...
	const UINT wX, const UINT wY)	//(in) [default=(-1,-1)]
{
	...
	if (bPlayerIsAnsweringQuestion)
	{
		...
		bool bPlayerIsMonsterTarget = this->swordsman.IsTarget();
		if (this->swordsman.wPlacingDoubleType)
		{
			...
			*pbMonstersKilled = 0;
			...
			if (CueEvents.HasOccurred(CID_RoomExitLocked) || CueEvents.HasOccurred(CID_BumpedMasterWall))
			{
				...
				//It should be impossible for player to exit a room
				//or bump a master wall during a cutscene.
			...
			if (bPlayerLeftRoom)
			{
				...
				//If oremites temporarily took away sword, but then something covered it up
				//and sword came back, process a sword hit now.
				//NOTE: It won't be synched with other sword hits, but that's the way it goes.
				if (bEntityHasSword(this->swordsman.wAppearance) && !this->swordsman.HasSword())
				{
					SetPlayerSwordSheathed();
					if (this->swordsman.HasSword())
					{
						const UINT wSX = this->swordsman.wSwordX;
						const UINT wSY = this->swordsman.wSwordY;
	...
	//If player died, check for clone to activate (for Beethro only).
	if (CueEvents.HasAnyOccurred(IDCOUNT(CIDA_PlayerDied), CIDA_PlayerDied))
	{
	...
	if (CueEvents.HasOccurred(CID_CheckpointActivated))
	{
					...
					this->wLastCheckpointY != this->swordsman.wY))
		{
			...
			if (!pMonster || (pMonster->wType != M_MIMIC && pMonster->wType != M_FEGUNDO))
				this->wLastCheckpointX = this->wLastCheckpointY = NO_CHECKPOINT;
		}
	}
	...
	if (!this->swordsman.wPlacingDoubleType && this->pRoom->SwordfightCheck())
...
//Params:
	...
	CMonster *pDouble)      //(in)   If NULL (default) this call is checking the
								...
								//    that double's sword.
{
	if (!this->pRoom->IsValidColRow(wSX, wSY)) return;

	CEntity *pKillingEntity;
	if (pDouble)
		pKillingEntity = pDouble;
	else
		pKillingEntity = &this->swordsman;
				...
				this->pRoom->IsMonsterOfTypeAt(M_NEATHER, wSX, wSY, true))))
	{
		...
		if (pMonster->OnStabbed(CueEvents, wSX, wSY))
		{
			if (CueEvents.HasOccurredWith(CID_MonsterDiedFromStab, pMonster))
			{
				const UINT wMonsterCount = this->pRoom->wMonsterCount;
				const UINT wType = pMonster->wType;

				//Store info about stab effect.
				pMonster->SetKillInfo(wSwordMovement);

				if (wType == M_ROCKGOLEM)
				{
					...
					//Monster was disabled previously, but should not be removed
					//from the room.
					TallyKill(wType);
					...
					if (this->pRoom->wMonsterCount < wMonsterCount)
					{
						TallyKill(pMonster->wType);
						if (!pDouble)
						{
							//Add to "swordsman tired" tally.
							++this->wMonstersKilledRecently;
							++this->monstersKilled[this->wTurnNo % TIRED_TURN_COUNT];
						}
					}
			...
			if (bIsTar(this->pRoom->GetTSquare(wSX, wSY)) &&
					(pMonster->wType == M_TARMOTHER || pMonster->wType == M_GELMOTHER ||
					(pMonster->wType == M_MUDMOTHER &&
					this->pRoom->StabTar(wSX, wSY, CueEvents, false))))
				//Mark tarstuff under mother for removal.
				//Mud is only removed if it is vulnerable here.
				this->simulSwordHits.push_back(CMoveCoord(wSX,wSY, wSwordMovement));

			//When player attacks a non-friendly entity...
			if (!pPlayerDouble && !bFriendly)
				this->swordsman.bIsTarget = true; //non-Beethro player is acquired as a target
		} else {
			//Monster was stabbed but not killed.
			if (CueEvents.HasOccurred(CID_MonsterPieceStabbed) && //player moves first,
											//so nothing else could have generated this event
					!pMonster->IsFriendly() && !pPlayerDouble)
				this->swordsman.bIsTarget = true; //non-Beethro player is acquired as a target
		}
	}
	...
	switch (wTileNo)
	{
		...
		case T_BOMB:
		   //Explode bomb immediately (kills swordsman).
		...
		case T_MIRROR:
		{
			...
			if (pPlayerDouble)
			{
				...
				wSrcX = this->swordsman.wX;
				wSrcY = this->swordsman.wY;
			}
	...
	if (pDouble)
	{
				...
				&& this->wTurnNo) //don't kill player on room entrance
		{
			...
			SetDyingEntity(&this->swordsman, pDouble);
			CueEvents.Add(CID_MonsterKilledPlayer, pDouble);
		}
	}
}
...
//If a checkpoint exists at (wX, wY), cue checkpoint save.
{
	...
	if (CueEvents.HasOccurred(CID_CheckpointActivated)) return;
...
//Saves the current game to the level-begin slot for this level.
{
	...
	ASSERT(this->swordsman.bSwordOff == this->bStartRoomSwordOff);
...
//*****************************************************************************
void CCurrentGame::SetCloneSwordsSheathed()
...
//Currently, this is based on whether player is standing on oremites or in shallow water.
{
	...
	this->swordsman.bSwordSheathed = wOSquare == T_GOO;
	return this->swordsman.bIsHiding || this->swordsman.bSwordSheathed;
}
...
//Currently, this includes sword-wielding and flying (based on player role) attributes.
{
	...
	if (this->wTurnNo > 0) //...except on room entrance
	{
				...
				pMonster = pMonster->pNext)
		{
			...
			{
				case M_CLONE:
					if (room.GetOSquare(pMonster->wX, pMonster->wY) == T_PRESSPLATE && !pMonster->IsFlying())
...
const
{
	...
	while (true)
	{
		...
		if (bIsTunnel(this->pRoom->GetOSquare(wX,wY)))
		{
			...
			if (this->pRoom->IsMonsterSwordAt(wX,wY) ||
					bIsTar(tTile) || bIsBriar(tTile) ||
					this->pRoom->GetMonsterAtSquare(wX,wY) != NULL)
			{
...
//Returns: whether the end of the stairs have been reached.
{
	...
	if (this->pRoom->IsValidColRow(this->swordsman.wX, this->swordsman.wY + yOffset))
	{
		if (pRoom->GetOSquare(this->swordsman.wX, this->swordsman.wY + yOffset) == wOSquare)
		{
			...
			if (!this->swordsman.HasSword()) //swordless, the player faces the direction of movement
...
//Currently expects only PlayerDoubles (Clones and Stalwart2) to be placed.
{
	...
	if (!this->pRoom->GetNearestEntranceTo(wHornX, wHornY, eMovement, wX, wY))
	{
		...
		if (wNewOSquare == T_PRESSPLATE &&
				(pDouble->wType != M_CLONE || !bIsEntityFlying(this->swordsman.wAppearance)))
			this->pRoom->ActivateOrb(pDouble->wX, pDouble->wY, CueEvents, OAT_PressurePlate);

		if (pDouble->wType == M_CLONE)
		{
			pDouble->SetSwordSheathed(); //Need to set sheath status before using items
			const bool bSmitemaster = bIsSmitemaster(this->swordsman.wAppearance);
			const bool bCanGetItems = bSmitemaster || this->swordsman.bCanGetItems;
			...
			switch(wNewTSquare)
			{
				...
				case T_TOKEN:
					this->pRoom->ActivateToken(CueEvents, pDouble->wX, pDouble->wY);
					break;

				case T_FUSE:
					//Light the fuse.
					if (this->swordsman.CanLightFuses())
						this->pRoom->LightFuse(CueEvents, pDouble->wX, pDouble->wY,
   ...
   if (nCommand==CMD_WAIT || nCommand==CMD_DOUBLE) // Create a new Double
   {
				...
				this->swordsman.wDoubleCursorY, this->swordsman.wPlacingDoubleType))
      {
         ...
         if (pDouble) //Yes, adding a Double worked.
         {
				...
				if (this->pRoom->GetTSquare(pDouble->wX, pDouble->wY) == T_TOKEN)
					this->pRoom->ActivateToken(CueEvents, pDouble->wX, pDouble->wY);
				pDouble->SetSwordSheathed();
				if (pDouble->HasSword())
				{
					ProcessSwordHit(pDouble->GetSwordX(), pDouble->GetSwordY(), CueEvents, pDouble);
					//Check whether sword hit tarstuff.
					ProcessSimultaneousSwordHits(CueEvents);  //destroy stabbed tarstuff now
				}
	...
	CCueEvents &CueEvents)  //(in/out)  List of events that can be handled by caller.
{
	...
	const bool bClosed = room.DoesSquareContainPlayerObstacle(wX, wY, wO, bIgnored) ||
			IsPlayerAt(wX, wY) || IsPlayerSwordAt(wX, wY);
	...
	CCueEvents &CueEvents)  //(in/out)  List of events that can be handled by caller.
{
	...
	while (pMonster)
	{
		...
		if (!bPlatformCheck && pMonster->wProcessSequence > SPD_PDOUBLE)
		{
			CPlatform::checkForFalling(this->pRoom, CueEvents);
			bPlatformCheck = true;
		}
		//Brains detect players once doubles are done moving.
		if (!bBrainCheck && pMonster->wProcessSequence > SPD_PDOUBLE)
		{
			...
			switch (pMonster->wType)
			{
				...
				case M_SERPENT:
					if (CueEvents.HasOccurredWith(CID_SnakeDiedFromTruncation, pMonster))
					{
						...
						this->pRoom->KillMonster(pMonster, CueEvents);
						TallyKill(pMonster->wType);  //counts as a kill
					}
				break;
				case M_REGG:
							...
							//    data on return.
{
	...
	while (!this->simulSwordHits.empty()) {
		//Remove tar now.
		CMoveCoord& moveCoord = this->simulSwordHits.back();
		this->pRoom->StabTar(moveCoord.wX, moveCoord.wY, CueEvents, true, moveCoord.wO);
		this->simulSwordHits.pop_back();
	}
}
							...
							//    data on return.
{
	...
	switch (nCommand)
	{
		...
		case CMD_C:
			if (this->swordsman.HasSword())
				CueEvents.Add(CID_SwingSword,
					new CAttachableWrapper<UINT>(this->swordsman.wO), true);	//comes first
		...
		case CMD_CC:
			...
			if (this->swordsman.HasSword())
				CueEvents.Add(CID_SwingSword,
					new CAttachableWrapper<UINT>(this->swordsman.wO), true);	//comes after
			break;
		//Move swordsman.
		case CMD_NW: dx = dy = -1; break;
		case CMD_N: dy = -1; break;
	...
	const bool bCanEnterTunnel = bIsMonsterTarget(this->swordsman.wAppearance) || this->swordsman.bCanGetItems;
	if (bIsTunnel(wOTileNo) && bCanEnterTunnel)
	{
		switch (wOTileNo)
		{
	...
	if (!this->pRoom->IsValidColRow(this->swordsman.wX + dx, this->swordsman.wY + dy))
	{
	...
	if (this->bRoomExitLocked)
	{
		...
		if (bIsStairs(this->pRoom->GetOSquare(this->swordsman.wX + dx, this->swordsman.wY + dy)))
		{
			CueEvents.Add(CID_RoomExitLocked);
			return;
		}
	...
	if (this->pRoom->DoesSquareContainPlayerObstacle(
			this->swordsman.wX + dx, this->swordsman.wY + dy, wMoveO, bMonsterObstacle))
	{
		...
		const UINT wNewOTile = this->pRoom->GetOSquare(this->swordsman.wX + dx,
				this->swordsman.wY + dy);
		const UINT wNewFTile = this->pRoom->GetFSquare(this->swordsman.wX + dx,
				this->swordsman.wY + dy);
		const UINT wNewTTile = this->pRoom->GetTSquare(this->swordsman.wX + dx,
				this->swordsman.wY + dy);
			...
			case T_DOOR_Y:
				if (bIsBeethroDouble(this->swordsman.wAppearance))
				{
					...
					if (pMonster)
					{
						...
						pHalph->RequestOpenDoor(this->swordsman.wX + dx,
								this->swordsman.wY + dy, CueEvents);
					} else
						CueEvents.Add(CID_HitObstacle,
								new CMoveCoord(this->swordsman.wX + dx,
										this->swordsman.wY + dy, wMoveO), true);
				}
			break;
			case T_WALL_M:
		...
		switch (wNewTTile)
		{
			...
			case T_ORB:
				if ((bIsHuman(this->swordsman.wAppearance) &&
					...
					this->pRoom->ActivateOrb(this->swordsman.wX + dx,
						this->swordsman.wY + dy, CueEvents, OAT_Player);
			...
			case T_MIRROR:
			{
				//Swordsman ran into mirror.  Push if possible.
				const UINT destX = this->swordsman.wX + dx, destY = this->swordsman.wY + dy;
...
CheckMonsterLayer:
		...
		CMonster *pMonster = this->pRoom->GetMonsterAtSquare(
				this->swordsman.wX + dx, this->swordsman.wY + dy);
		...
		if (pMonster && bMonsterObstacle)
		{
			...
			if (bNotAnObstacle &&
					...
					!this->pRoom->IsMonsterSwordAt(this->swordsman.wX + dx, this->swordsman.wY + dy))
			{
				...
				if (bPushingMirror)
					this->pRoom->PushObject(this->swordsman.wX + dx, this->swordsman.wY + dy,
							this->swordsman.wX + dx*2, this->swordsman.wY + dy*2, CueEvents);
				goto MakeMove;
			}
		}
...
MakeMove:
	...
	const bool bSmitemaster = bIsSmitemaster(this->swordsman.wAppearance);
	const bool bCanGetItems = bSmitemaster || this->swordsman.bCanGetItems;
	const bool bCanDropTrapdoors = bCanGetItems && this->swordsman.HasSword();
	...
	if (!bEnteredTunnel && nCommand != CMD_C && nCommand != CMD_CC)
		SetPlayer(this->swordsman.wX + dx, this->swordsman.wY + dy);
	...
	const bool bSwordWasSheathed = !this->swordsman.HasSword();
			...
			this->swordsman.CanStepOnMonsters()))       //player in monster-role attacked another monster
	{
		...
		this->swordsman.bIsTarget = true;  //monsters will attack a non-Beethro player now
	}
	...
	if (wNewOSquare == T_PRESSPLATE && bMoved &&
			!bIsEntityFlying(this->swordsman.wAppearance))
		this->pRoom->ActivateOrb(this->swordsman.wX, this->swordsman.wY, CueEvents, OAT_PressurePlate);

	//Check for reading a scroll.
	UINT wNewTSquare = this->pRoom->GetTSquare(this->swordsman.wX, this->swordsman.wY);
	...
	switch (wNewTSquare)
	{
		...
		default:        //normal step (appropriate type of footfall)
			if (bMoved && !bIsEntityFlying(this->swordsman.wAppearance)) {
				if (bIsWater(wNewOSquare))
	...
	//Check for things that the player's sword could hit.
	if (this->swordsman.HasSword())
	...
	switch (wNewTSquare)
	{
		case T_FUSE:
			//Light the fuse.
			if (bCanGetItems)
				this->pRoom->LightFuse(CueEvents, this->swordsman.wX, this->swordsman.wY,
						this->wTurnNo > 0);  //start burning right away if lit on room entrance
		break;
	}
...
//Params:
	...
	const UINT wEntrance,   //if set, go to this level entrance [default=-1]
	const bool bSkipEntranceDisplay) //[default=false]
{
	ASSERT(bIsStairs(this->pRoom->GetOSquare(this->swordsman.wX, this->swordsman.wY)) ||
			wEntrance != (UINT)-1);
	...
	if (wEntrance == (UINT)-1) //was exit destination not specified?
	{
		for (UINT i=0; i<this->pRoom->Exits.size(); ++i)
		{
			CExitData *pStairs = this->pRoom->Exits[i];
			if (this->swordsman.wX >= pStairs->wLeft && this->swordsman.wX <= pStairs->wRight &&
					this->swordsman.wY >= pStairs->wTop && this->swordsman.wY <= pStairs->wBottom)
			{
				//Robustness measure: verify this entrance record actually exists.
				if (pStairs->dwEntranceID && !this->pHold->GetEntrance(pStairs->dwEntranceID))
				{
		...
		if (dwEntranceID == (UINT)-1) //no exit destination defined by exits
			dwEntranceID = 0;          //so end the hold
	}
...
//unsuccessful (ProcessPlayer() should keep player in current room).
{
	...
	if (!this->swordsman.HasSword() && wExitDirection != NO_ORIENTATION)
...
//			this->pRoom->ToggleTiles(T_DOOR_C, T_DOOR_CO); //blue door
	...
	if (bRoomWasSecret && bConquered && !this->bHoldMastered)
	{
		this->bHoldMastered = g_pTheDB->Holds.IsHoldMastered(this->pHold->dwHoldID, g_pTheDB->GetPlayerID());
		if (this->bHoldMastered)
	...
	switch (iQuestion->eType)
	{
		case MMT_YESNO:
			...
			if (iQuestion->pSender && iQuestion->pSender->OnAnswer(nCommand, CueEvents))
			{
				...
				//and ProcessMonsters() or ProcessSwordHit().  I happen to know that
...
//*****************************************************************************
void CCurrentGame::SetLevelStats()
	...
	const bool bResetCommands) //(in) [default = true]
{
	...
	if (!bWasRoomConquered && !wMonsterCountAtStart && !this->pRoom->bHasConquerToken)
	{
		SetCurrentRoomConquered();
	...
	ProcessSimultaneousSwordHits(CueEvents);  //destroy simultaneously-stabbed tar
	this->pRoom->KillGhostsOutsideWall(CueEvents); //check again if doors have changed
	SetPlayerMood(CueEvents);
	this->pRoom->ResetMonsterFirstTurnFlags();
	//Check whether swordsman's first step left room in a conquer-pending state.
	if ((wMonsterCountAtStart || this->pRoom->bHasConquerToken) && //room needs conquering
			IsCurrentRoomPendingExit())
	{
		ToggleGreenDoors();
	...
	//Add this room to room tally.
	AddRoomsToPlayerTally();

	//Check for Hold Mastered on entering a cleared-on-entrance secret room
	if (this->pRoom->bIsSecret && !this->bHoldMastered && IsCurrentRoomPendingExit())
	{
		this->bHoldMastered = g_pTheDB->Holds.IsHoldMastered(this->pHold->dwHoldID, this->dwPlayerID);
		if (this->bHoldMastered)
			CueEvents.Add(CID_HoldMastered);
	}
...
//Sets player-related members to start of room.
{
	...
	this->swordsman.bSwordOff = this->bStartRoomSwordOff;
...
//*****************************************************************************
void CCurrentGame::TallyKill(const UINT wType)
...
//Switch player and clone's positions.
{
	...
	ASSERT(!this->pRoom->GetMonsterAtSquare(this->swordsman.wX, this->swordsman.wY));
...
//Returns: whether a new game state snapshot should be taken and stored
{
	...
	static const UINT MIN_TURNS_PER_SNAPSHOT = 30;
	...
	static const UINT MAX_SNAPSHOTS = 30;
...
//Only sends the minimum necessary XML, and doesn't care if the server receives it or not
{
	...
	if (bSubmitScores)
	{
		...
		if (g_pTheNet->IsLocalHold(this->pHold->dwHoldID) && g_pTheNet->IsLoggedIn())
		{
			...
			CIDSet roomIDs(this->dwRoomID);
			CDbRefs dbRefs(V_Rooms, roomIDs);
			g_pTheDB->Rooms.ExportXML(this->dwRoomID, dbRefs, roomRef, true);
			roomRef += CDbXML::getXMLfooter();

			g_pTheNet->UploadExploredRoom(roomRef);
		}
	}
	delete pPlayer;
}
...
//DemoID of new Demos record.
{
	...
	//Demo hi-score submission for published holds.
	CDbPlayer *pPlayer = g_pTheDB->GetCurrentPlayer();
	ASSERT(pPlayer);
	const bool bSubmitScores = pPlayer->Settings.GetVar(useInternetStr, false);
	if (bSubmitScores && this->wPlayerTurn) //don't accept 0-move demos
	{
		...
		if (g_pTheNet->IsLocalHold(this->pHold->dwHoldID))
		{
			...
			if (CDbXML::ExportXML(V_Demos, ids, text, UINT(-1))) //no multi-room demos
			{
				...
				CCurrentGame::demosForUpload.push(new DEMO_UPLOAD(text,
						(this->conquerTokenTurn != NO_CONQUER_TOKEN_TURN) ?
								this->conquerTokenTurn : this->wPlayerTurn,
		...
		{
			pPlayer->Settings.SetVar(playerScoresOld, (BYTE)1);
			pPlayer->Update();
		}
	}
	delete pPlayer;
