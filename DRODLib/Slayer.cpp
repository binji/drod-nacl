// $Id: Slayer.cpp 9999 2012-03-24 17:47:53Z mrimer $
...
//Params:
	...
	: CPlayerDouble(type, pSetCurrentGame,
	...
	CCueEvents &CueEvents, const UINT wX, const UINT wY)
{
	...
	return CMonster::OnStabbed(CueEvents, wX, wY);
}
							...
							//sound or graphical effects.
{
	...
	if (HasSword())
	{
		...
		if (wDistMySwordToTarget == 0)
			return;	//stabbing the target
		if (wDistMySwordToTarget == 1)
		{
			...
			const bool bFoundDir = IsOpenMove(dx, dy) && !DoesSquareRemoveSword(this->wX + dx, this->wY + dy);
			if (bFoundDir)
			{
			...
			if (wMyDistFromTarget == 1 &&
					(GetSwordX() == this->wTX || GetSwordY() == this->wTY))	//one rotation is sufficient
			{
	...
	if (this->wDistToTarget == 1 &&
	    !(bTargetIsPlayer && !player.IsStabbable()))  // can't step on wubba player
	{
		//Step on target.
		int dxFirst, dyFirst;
...
{
	if (!this->bMovingWisp)
		return CPlayerDouble::DoesSquareContainObstacle(wCol, wRow);
	...
	if (pMonster)
	{
		...
		if (pMonster->IsLongMonster() || pMonster->IsPiece() ||
				pMonster->wType == M_ROCKGOLEM || pMonster->wType == M_GUARD ||
...
{
	return CPlayerDouble::IsOpenMove(dx,dy) &&
...
//Removes all elements of the wisp list after (or before) 'piece' (inclusive).
{
	...
	list<CMonsterPiece*> truncatedWisp;
	if (bKeepBack)
...
#define COMPARE_SCORE { \
	...
	fAttackDist = (float)nDist(GetSwordX(), GetSwordY(), this->wTX, this->wTY); \
	CALC_SCORE \
}
