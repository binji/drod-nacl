// $Id: DbPackedVars.cpp 9742 2011-10-22 16:12:27Z mrimer $
...
int CDbPackedVars::GetVar(const char *pszVarName, int nNotFoundValue)
...
UINT CDbPackedVars::GetVar(const char *pszVarName, UINT wNotFoundValue)
...
char CDbPackedVars::GetVar(const char *pszVarName, char cNotFoundValue)
...
const char * CDbPackedVars::GetVar(const char *pszVarName, const char *pszNotFoundValue)
//Overload for returning a char * value.
{
...
const WCHAR * CDbPackedVars::GetVar(const char *pszVarName, const WCHAR *pwczNotFoundValue)
...
BYTE CDbPackedVars::GetVar(const char *pszVarName, BYTE ucNotFoundValue)
...
bool CDbPackedVars::GetVar(const char *pszVarName, bool bNotFoundValue)
...
//*******************************************************************************************
UNPACKEDVARTYPE CDbPackedVars::GetVarType(const char *pszVarName)
...
{
	if (strcmp(pszVarName, "ExportPath" ) == 0)
		return UVT_char_string;
	if (
		strcmp(pszVarName, "AutoSaveOptions" ) == 0 ||
		strcmp(pszVarName, "hd" ) == 0)
      return UVT_uint;
	if (
	  strcmp(pszVarName, "MusicVolume" ) == 0 ||
	  strcmp(pszVarName, "SoundEffectsVolume" ) == 0 ||
	  strcmp(pszVarName, "RepeatRate" ) == 0
	  )
      return UVT_byte;
	if (
	  strcmp(pszVarName, "AutoSave" ) == 0 ||
	  strcmp(pszVarName, "Fullscreen" ) == 0 ||
	  strcmp(pszVarName, "ItemTips" ) == 0 ||
	  strcmp(pszVarName, "Language" ) == 0 ||
	  ...
	  strcmp(pszVarName, "Music" ) == 0 ||
	  strcmp(pszVarName, "Restart" ) == 0 ||
	  strcmp(pszVarName, "ShowCheckpoints" ) == 0 ||
	  strcmp(pszVarName, "ShowErrors" ) == 0 ||
	  strcmp(pszVarName, "SoundEffects" ) == 0 ||
