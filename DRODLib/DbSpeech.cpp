// $Id: DbSpeech.cpp 10060 2012-03-31 13:47:22Z mrimer $
	...
	bool &bSaveRecord)      //(out) whether record should be saved
{
	...
	{
		case P_SpeechID:
         this->dwSpeechID = static_cast<UINT>(atol(str));
			if (!this->dwSpeechID)
		...
		case P_Character:
			this->wCharacter = static_cast<UINT>(atoi(str));
			break;
		case P_Mood:
			this->wMood = static_cast<UINT>(atoi(str));
			break;
		case P_DataID:
		...
		case P_Delay:
			this->dwDelay = static_cast<UINT>(atol(str));
			break;
		default:
			return MID_FileCorrupted;
	}
