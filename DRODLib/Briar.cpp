// $Id: Briar.cpp 9998 2012-03-24 17:33:24Z mrimer $
...
//Returns: whether the briar's turn is not done
{
	...
	ASSERT(room.GetTSquare(wClosestX,wClosestY) == T_FLOW_EDGE);
	room.Plot(wClosestX,wClosestY,T_FLOW_INNER);
			...
			for (UINT i=0; i<NUM_NEIGHBORS; ++i)
			{
				...
				switch (wOTile)
				{
					...
					case T_PIT: case T_PIT_IMAGE:
						...
						//Briars into hole.
						bAdjacentPit = true;
						if (wSrcTile == T_FLOW_INNER) //this tile becomes an edge again
						{
							this->pRoom->Plot(tile->wX,tile->wY,T_FLOW_EDGE);
				...
				this->pRoom->Plot(wX,wY,T_FLOW_EDGE);
				edge.insert(wX,wY);
...
//*****************************************************************************
CCoordSet CBriars::getEdgeTilesFor(const UINT wX, const UINT wY) const
...
//Add a briar root at (x,y)
{
	...
	for (UINT i=0; i<NUM_NEIGHBORS; ++i)
	{
		...
		switch (wOTile)
		{
			...
			//case T_WATER: case T_SHALLOW_WATER: case T_PLATFORM_W:
				bAdjacentPit = true;
			break;
		}
	...
	CCueEvents &CueEvents)     //(in/out)
{
	...
	if (this->bRecalc) //need to recompute connected components
	{
		...
		if (briarMask.empty())
		{
			briarMask.set(T_FLOW_SOURCE);
			briarMask.set(T_FLOW_INNER);
			briarMask.set(T_FLOW_EDGE);
		}
		...
		for (briar = this->briars.begin(); briar != this->briars.end(); ++briar)
		{
			...
			this->pRoom->GetConnectedTiles(briarObj.wX, briarObj.wY, briarMask, true, preBriarComponent);
			...
			for (CCoordSet::const_iterator tile=preBriarComponent.begin(); tile!=preBriarComponent.end(); ++tile)
			{
				...
				if (oldBriarIndices.GetAt(tile->wX, tile->wY) == oldIndex)
				{
					...
					if (this->pRoom->GetTSquare(tile->wX, tile->wY) == T_FLOW_EDGE)
	...
	do {
		...
		for (briar = this->briars.begin(); briar != this->briars.end(); ++briar)
		{
			...
			{
				expand(CueEvents, pBriar->wComponentIndex);
