// $Id: DbPackedVars.h 9742 2011-10-22 16:12:27Z mrimer $
...
public:
	...
	void *         GetVar(const char *pszVarName, const void *pNotFoundValue = NULL);
	const char *   GetVar(const char *pszVarName, const char *pszNotFoundValue = NULL);
	const WCHAR *  GetVar(const char *pszVarName, const WCHAR *pwczNotFoundValue = NULL);
	int            GetVar(const char *pszVarName, int nNotFoundValue = 0);
	UINT        GetVar(const char *pszVarName, UINT wNotFoundValue = 0);
	char        GetVar(const char *pszVarName, char cNotFoundValue = 0);
	BYTE        GetVar(const char *pszVarName, BYTE ucNotFoundValue = 0);
	bool        GetVar(const char *pszVarName, bool ucNotFoundValue = false);

	UNPACKEDVARTYPE GetVarType(const char *pszVarName);
	UINT       GetVarValueSize(const char *pszVarName);
...
private:
	...
	UNPACKEDVAR *   FindVarByName(const char *pszVarName);
	...
	std::map <string, UNPACKEDVAR*>::const_iterator lastQueryIter;
