// $Id: DbDemos.cpp 10065 2012-04-02 19:44:52Z schik $
...
{
	CIDList DemoStats;
	const bool bDemoCorrect = Test(DemoStats, bConsiderHoldMastered);
	...
	bool &bSaveRecord)      //(out) whether record should be saved
{
	...
	{
		case P_DemoID:
         this->dwDemoID = static_cast<UINT>(atol(str));
			if (!this->dwDemoID)
		...
		case P_EndTurnNo:
			...
			break;
		case P_NextDemoID:
			this->dwNextDemoID = static_cast<UINT>(atol(str));
		...
		case P_Checksum:
			...
			break;
		case P_Flags:
			this->dwFlags = static_cast<UINT>(atol(str));
			break;
		default:
			return MID_FileCorrupted;
	}
