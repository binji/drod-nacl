// $Id: Splitter.cpp 9742 2011-10-22 16:12:27Z mrimer $
	...
	const bool bBreakOverPit) //[default=false]
{
	...
	room.GetSwordCoords(swordCoords); //resets var
	...
	CCueEvents &CueEvents) //(in/out)
{
	...
	if (pGiant->IsPiece())
	{
		CMonsterPiece *pPiece = DYN_CAST(CMonsterPiece*, CMonster*, (CMonster*)pGiant);
		pGiant = pPiece->pMonster;
	}
