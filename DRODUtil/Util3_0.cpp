// $Id: Util3_0.cpp 9786 2011-11-30 23:36:58Z mrimer $
...
const
{
	...
	if (!CFiles::DoesFileExist(wstrMasterDatFilepath.c_str()))
	{
		...
		printf("FAILED--Couldn't find data to delete at %s." NEWLINE, path);
#endif
		return false;
	}
	...
	if (!bDeleteSuccess)
	{
		...
		string strHoldFilepath;
		UnicodeToAscii(wstrMasterDatFilepath, strHoldFilepath);
...
const
{
	...
	if (!IsPathValid(this->strPath.c_str()))
	{
		...
		char path[strPath.length()+1];
		UnicodeToAscii(strPath, path);
		printf("FAILED--Destination path (%s) is not a valid path." NEWLINE, path);
#endif
		return false;
	}
	...
	if (DoesFileExist(wstrMasterDatFilepath.c_str()))
	{
		...
		char path[strPath.length()+1];
		UnicodeToAscii(strPath, path);
		printf("FAILED--data already exists at %s." NEWLINE, path);
#endif
		return false;
	}
...
const
{
  ...
  bool bImportMIDs = Options.GetSize()==0 || Options.Exists(wt);
	...
	if (!IsPathValid(pszSrcPath))
	{
		...
		printf("FAILED--Source path (%s) is not a valid path." NEWLINE, path);
#endif
		return false;
	}
	...
	if (!DoesFileExist(wstrMasterDatFilepath.c_str()))
	{
		...
		char path[strPath.length()+1];
		UnicodeToAscii(strPath, path);
		printf("FAILED--Storage files not found at %s." NEWLINE, path);
#endif
		return false;
	}
	...
	//Populate hold database with messages needed and maybe basic text.
	{
		...
		UnicodeToAscii(wstrMasterDatFilepath.c_str(), filename);
