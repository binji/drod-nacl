// $Id: HoldSelectScreen.cpp 9929 2012-03-10 15:21:28Z TFMurphy $
...
//Returns: true if hold was processed, false if all holds in list have been processed
{
	...
	if (this->wProcessingHoldLine >= this->pFullHoldList->GetItemCount())
	...
	if (!dwHoldID)
		return true; //hold not installed -- change nothing

	ASSERT(wLineNo < this->holdInfo.size());

	//Mark whether hold is in progress.
	const UINT dwOldHoldID = g_pTheDB->GetHoldID();
	g_pTheDB->SetHoldID(dwHoldID);
	const UINT dwContinueGameID = g_pTheDB->SavedGames.FindByContinue();
	if (dwContinueGameID)
		this->holdInfo[wLineNo].bInProgress = true;
	...
	if (!dwEndHoldID)
		return true; //not conquered -- change nothing

	SetCursor(CUR_Wait);

	this->holdInfo[wLineNo].bConquered = true;
	...
	wStr += wszRightParen;
	if (wPercent == 100)
		this->holdInfo[wLineNo].bMastered = true;
...
void CHoldSelectScreen::DeleteSelectedHolds()
{
	...
	for (CIDSet::const_iterator id = holdIDs.begin(); id != holdIDs.end(); ++id)
	{
		...
		if (pHold->status == CDbHold::GetOfficialHoldStatus() || pHold->status == CDbHold::Tutorial)
		{
...
//Sets export permissions and download availability.
{
	...
	this->pDeleteButton->Enable(bHoldAuthor || pHold->status == CDbHold::Homemade ||
			pHold->status == CDbHold::KDD ||
			pHold->status == CDbHold::JtRH ||
			pHold->status == CDbHold::TCB ||
...
void CHoldSelectScreen::SetHoldFilter()
{
	...
	for (UINT i=0; i<this->holdInfo.size(); ++i)
	{
		const HoldInfo& h = this->holdInfo[i];
		bool bShow = true;
		...
		switch (this->filter)
		{
			default:
			case F_ALL: break;
			case F_MINE: bShow = h.bMine; break;
			case F_NEW: bShow = !h.bInProgress && h.bLocalHold; break;
			case F_INPROGRESS: bShow = h.bInProgress; break;
			case F_CONQUERED: bShow = h.bConquered; break;
			case F_UNCONQUERED: bShow = !h.bConquered && h.bLocalHold; break;
			case F_MASTERED: bShow = h.bMastered; break;
			case F_UNMASTERED: bShow = !h.bMastered && h.bLocalHold; break;
			case F_CNET: bShow = h.bCaravelNetHold; break;
			case F_NONCNET: bShow = !h.bCaravelNetHold; break;
			case F_BETA: bShow = h.bBeta; break;
			case F_UPDATED: bShow = h.bUpdated; break;
			case F_OFFICIAL: bShow = h.status != CDbHold::Homemade &&
					h.status != CDbHold::Tutorial && h.status != CDbHold::NoStatus;
			break;
		}
		if (bShow)
		{
			const UINT dwKey = this->pFullHoldList->GetKeyAtLine(i);
			...
			const UINT insertedAtIndex = this->pHoldListBoxWidget->AddItem(dwKey,
					this->pFullHoldList->GetTextAtLine(i),
					!bLocalHold, -1, true); //place non-local holds last
			this->pHoldListBoxWidget->EnableItemAtIndex(insertedAtIndex);

			if (h.status == CDbHold::Official || h.status == CDbHold::JtRH ||
					h.status == CDbHold::KDD || h.status == CDbHold::TCB ||
					h.status == CDbHold::GatEB) //make official holds stand out
			{
...
//Put the holds in the DB into the hold list box.
{
	...
	vector<CNetMedia*>& cNetMedia = g_pTheNet->GetCNetMedia();
	CIDSet& betaHolds = g_pTheNet->GetBetaHolds();
	CIDSet& localCNetHolds = g_pTheNet->GetLocalHolds();
	CIDSet& updatedHolds = g_pTheNet->GetUpdatedHolds();
	...
	BEGIN_DBREFCOUNT_CHECK;
	{
		...
		for (CDbHold *pHold = db.Holds.GetFirst(true); pHold != NULL; pHold = db.Holds.GetNext())
		{
			...
			HoldInfo holdinfo;
			holdinfo.bMine = pHold->dwPlayerID == playerID;
			holdinfo.bCaravelNetHold = localCNetHolds.has(pHold->dwHoldID);
			holdinfo.bBeta = betaHolds.has(pHold->dwHoldID);
			holdinfo.bLocalHold = true;
			holdinfo.status = pHold->status;
			holdinfo.bUpdated = updatedHolds.has(pHold->dwHoldID);
			holdinfo.localHoldID = pHold->dwHoldID;
			holdinfo.holdNameText = static_cast<WSTRING>(pHold->NameText);
			...
			localHoldInfo.push_back(holdinfo);

			//Cross-reference hold's CaravelNet rating.
			CNetMedia *pHoldData = NULL;
			const int cNetMediaIndex = g_pTheNet->getMediaIndexWithLocalHoldID(holdinfo.localHoldID);
			if (cNetMediaIndex >= 0)
		...
		//Display non-local holds in sorted (descending) order.
		...
		for (hold = holdSortMap.rbegin(); hold != holdSortMap.rend(); ++hold)
		{
			...
			const HoldInfo& holdinfo = localHoldInfo[index];

			WSTRING wStr = holdinfo.holdNameText;
			...
			const UINT wInsertedAtIndex = this->pFullHoldList->AddItem(holdinfo.localHoldID, wStr.c_str());
			this->holdInfo.insert(this->holdInfo.begin() + wInsertedAtIndex, holdinfo);
		}
		...
		{
			CNetMedia& holdData = *(cNetMedia[wIndex]);

			//Skip non-holds.
			if (holdData.mediaType != MT_Hold) continue;

			//Skip any holds without a server ID.
			if (!holdData.lHoldID) continue;

			//Skip holds from a future version of the game.
			if (holdData.wVersion >= 500) continue;

			//Show non-installed CaravelNet holds.
			if (!holdData.localHoldID)
			{
				//Sort.
				const double fVal = getSortVal(eSortType, &holdData, wIndex);
				holdSortMap.insert(std::make_pair(fVal, wIndex));
			}
			//else it was already added to list as a local hold above
		}

		//Sort non-local holds (in descending order).
		for (hold = holdSortMap.rbegin(); hold != holdSortMap.rend(); ++hold)
		{
			//Get all items with this value.
			UINT index = hold->second;
			CNetMedia& holdData = *(cNetMedia[index]);
			WSTRING holdName = (const WCHAR*)holdData.HoldNameText;
			const UINT wInsertedAtIndex = this->pFullHoldList->AddItem(
					0, holdName.c_str(), true, -1, true); //place after local holds
			HoldInfo holdinfo;
			holdinfo.bCaravelNetHold = true;
			holdinfo.bBeta = holdData.bBeta;
			holdinfo.status = holdData.status;
			holdinfo.version = holdData.wVersion;
