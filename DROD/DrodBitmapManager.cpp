// $Id: DrodBitmapManager.cpp 10032 2012-03-29 07:28:53Z mrimer $
...
//True if successful, false if not.
{
	...
	for (wI=TEXTURE_COUNT; wI--; )
	{
		if (wI == FLOOR_IMAGE) continue; //skip loading for FLOOR_IMAGE
	...
	if (this->pTextures[DEEP_MOSAIC]->format->Amask)
	{
		SDL_Surface *pDeepSurface = this->pTextures[DEEP_MOSAIC];
		this->pTextures[DEEP_MOSAIC] = SDL_DisplayFormat(pDeepSurface);
		SDL_FreeSurface(pDeepSurface);
	}
	if (this->pTextures[SHALLOW_MOSAIC]->format->Amask)
	{
		SDL_Surface *pShallowSurface = this->pTextures[SHALLOW_MOSAIC];
		this->pTextures[SHALLOW_MOSAIC] = SDL_DisplayFormat(pShallowSurface);
		SDL_FreeSurface(pShallowSurface);
	}
