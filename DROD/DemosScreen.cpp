// $Id: DemosScreen.cpp 9742 2011-10-22 16:12:27Z mrimer $
	...
	UINT dwDemoID)   //(in)   If 0L then no demo will be selected.
{
	...
	pDemo->GetNarrationText(wstrNarrationText, wEndTurnNo, true);
...
const
{
	static const char *tagStr[Demo_Tag_Count] = {
		...
		"RoomNotConquered", "RoomNotScorable"
	};
	...
	const XML_Char *name, const XML_Char **atts)
{
	...
	if (stricmp(name, "highscore")) return;   //not case sensitive
	...
	for (i = 0; atts[i]; i += 2) {
		...
		{
			case DemoID:
				pDemo->lDemoID = atol(str);
				break;
			case NumMoves:
				pDemo->wNumMoves = atol(str);
				break;
			case TimeElapsed:
				pDemo->wTimeElapsed = atol(str);
				break;
			case TimeReceived:
				pDemo->wTimeReceived = atol(str);
				break;
			case UserName:
			...
			case PlayerName:
				...
				break;
			case Created:
				pDemo->wCreated = atol(str);
				break;
			case RoomNotConquered:
				pDemo->bRoomNotConquered = true;
				break;
			case RoomNotScorable:
