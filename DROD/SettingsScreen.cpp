// $Id: SettingsScreen.cpp 10005 2012-03-24 18:08:02Z mrimer $
...
{	//numpad default
	...
	SDLK_w, SDLK_q, SDLK_r, SDLK_BACKSPACE, SDLK_KP_PLUS, SDLK_KP_PERIOD
},{	//laptop default
	SDLK_7, SDLK_8, SDLK_9, SDLK_u, SDLK_i, SDLK_o, SDLK_j, SDLK_k, SDLK_l,
	SDLK_w, SDLK_q, SDLK_r, SDLK_BACKSPACE, SDLK_0, SDLK_PERIOD
}};
...
const MESSAGE_ID COMMAND_MIDS[DCMD_Count] = {
	...
	MID_RestartRoom, MID_UndoMove, MID_BattleKey, MID_UseCommandKey
};
...
const UINT TAG_DEFAULT_DESKTOP = 1016;
const UINT TAG_DEFAULT_LAPTOP = 1017;
	...
	, pCaravelNetPasswordWidget(NULL)
{
	...
	const UINT CY_SOUND_FRAME = Y_SHOWSUBTITLES + CY_SHOWSUBTITLES + CY_SPACE;
...
//	const UINT CY_COMMANDS_FRAME = 540;
	...
	const UINT CX_REPEATRATE = CX_SPECIAL_FRAME - (5*CX_SPACE + 100);
	...
	static const UINT ButtonTags[BUTTON_COMMAND_COUNT] = {
		...
		TAG_UNDO_BUTTON, TAG_BATTLE_BUTTON, TAG_COMMAND_BUTTON
	};
...
#  define SETMISSING(name, value) if (!Settings.DoesVarExist(name)) Settings.SetVar(name, value);
	...
	SETMISSING("EyeCandy", BYTE(Metadata::GetInt(MetaKey::MAX_EYE_CANDY)));
	...
	SETMISSING(groupEditorMenuItems, false);
	...
	CDbPlayer *pPlayer)  //(in)   Player data.
{
	...
	bytValue = settings.GetVar("EyeCandy", g_pTheBM->eyeCandy);
	...
	pOptionButton->SetChecked(settings.GetVar(groupEditorMenuItems, false));
	//Check above line
	...
	CDbPlayer *pPlayer)  //(in/out)  Accepts loaded player, returns with members updated.
{
	...
	pSliderWidget = DYN_CAST(CSliderWidget*, CWidget*, GetWidget(TAG_ENVIRONMENT));
	settings.SetVar("EyeCandy", pSliderWidget->GetValue());
	...
	pOptionButton = DYN_CAST(COptionButtonWidget*, CWidget*,
			GetWidget(TAG_GROUPEDITORMENUITEMS));
	settings.SetVar(groupEditorMenuItems, pOptionButton->IsChecked());
...
//progress and best demo for each room.
{
	...
	this->pCurrentPlayer->Settings.SetVar(playerScoresOld, (BYTE)0);
