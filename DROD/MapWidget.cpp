// $Id: MapWidget.cpp 10032 2012-03-29 07:28:53Z mrimer $
								...
								//    the widget's rect.
{
	...
	SDL_Rect dest = {this->x + nOffsetX, this->y + nOffsetY, this->w, this->h};
...
//Returns: whether a room was pasted
{
	...
	if (this->bVacantRoom)
	{
		...
		if (!this->bCopyingRoom)
		{
			...
			this->pRoom->dwDataID = 0; //don't copy room media data here -- it's done in MergeHoldData below
			CDbRoom *pNewRoom = this->pRoom->MakeCopy(info);
			...
			if (bChangingLevels)
			{
				...
				if (bChangingHolds)
					pNewRoom->dwDataID = 0; //reset dangling floor image ID
			}
			pNewRoom->Update();
						...
						//    in determining pixels.
{
	...
	const bool bDarkened  = (this->pCurrentGame ?
			this->DarkenedRooms.has(pRoom->dwRoomID) : false);
	const bool bPendingExit = (this->pCurrentGame ?
	...
	while (pSeek != pStop)
	{
		...
		while (pSeek != pEndOfRow)
		{
			Color = GetMapColorFromTile(
				...
				bConquered, bDarkened, bPendingExit, bRoomRequired, bRoomSecret);
#if (GAME_BYTE_ORDER == GAME_BYTEORDER_LITTLE) && defined(__APPLE__)
			pSeek[2] = Color.byt1;
			pSeek[1] = Color.byt2;
			pSeek[0] = Color.byt3;
#else
	...
	const bool bRoomSecret)
{
	...
	switch (wOpaqueTile)
	{
		...
		case T_GOO:
			return m_arrColor[bPendingExit && !bRoomConquered ? MAP_LTGREEN : MAP_GOO];
		case T_HOT:
			return m_arrColor[bPendingExit && !bRoomConquered ? MAP_LTGREEN : MAP_ORANGE];
...
//2. Ensure the room's character script IDs are unique within the hold.
//2a. When scripted hold vars have different names than those in the destination hold,
//add these vars to the hold and link them properly to script references.
//3. Reset the room's floor image ID, since this isn't shared between holds.
//
//Params:
	CDbHold *pHold,      //(in/out) destination hold: entrances and scriptID updated
	CDbRoom *pRoom,      //(in/out) dataID and scriptIDs updated for membership in destination hold
...
const
{
	...
	//3. Update room's floor image ID.
	if (pRoom->dwDataID)
	{
		//Make copy of room image if hold doesn't already have an image object with matching name.
		const WSTRING dataName = g_pTheDB->Data.GetNameFor(pRoom->dwDataID);
		CDb db;
		CIDSet imageFormats = CBitmapManager::GetSupportedImageFormats();
		db.Data.FilterByHold(pHold->dwHoldID);
		db.Data.FilterByFormat(imageFormats);
		const UINT dataCopyID = db.Data.FindByName(dataName.c_str());
		if (dataCopyID)
		{
			//An object
			pRoom->dwDataID = dataCopyID;
		} else {
			CDbData::CopyObject(info, pRoom->dwDataID);

			//Update object's hold ID.
			CDbDatum *pNewData = g_pTheDB->Data.GetByID(pRoom->dwDataID);
			pNewData->dwHoldID = pHold->dwHoldID;
			pNewData->Update();
			delete pNewData;
		}
		bIDsUpdated = true;
