// $Id: NewPlayerScreen.cpp 9830 2012-02-07 19:36:30Z mrimer $
...
#if defined(EMBED_STYLES) && defined(CARAVELBUILD) && defined(BETA)
	if (!dwPlayerID)
...
//As soon as it closes, return from this screen.
{
	...
	if (GetDestScreenType() != SCR_None)
	{
...

		{
			...
			pPlayer->Settings.SetVar("EyeCandy", g_pTheBM->eyeCandy);

			if (wFPS < 10)
			{
				g_pTheBM->bAlpha = false;
				pPlayer->Settings.SetVar("Alpha", g_pTheBM->bAlpha);
			}
...
//Returns: ID of new player added
{
   ...
   if (dwAnswer == TAG_YES || dwAnswer == TAG_NO)
	...
	pPlayer->Settings.SetVar(playSessions, UINT(1));
	pPlayer->Update();
