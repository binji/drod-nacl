// $Id: PendingPlotEffect.cpp 9742 2011-10-22 16:12:27Z mrimer $
	...
	SDL_Surface* pDestSurface) //(in)
{
	...
	for (wY=wStartY; wY<=wEndY; ++wY)
		for (wX=wStartX; wX<=wEndX; ++wX)
		{
			...
			wTileNo = (bIsWall(wObjectNo) ? TI_WALL : TI_STAIRS_1);
	...
	SDL_Surface* pDestSurface) //(in)
{
	...
	SDL_Rect dest = {this->OwnerRect.x + wX * CBitmapManager::CX_TILE,
