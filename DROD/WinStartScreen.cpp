// $Id: WinStartScreen.cpp 9959 2012-03-17 23:20:49Z mrimer $
...
{
	const UINT dwOfficialHoldID = g_pTheDB->Holds.GetHoldIDWithStatus(CDbHold::GetOfficialHoldStatus());
	if (g_pTheDB->GetHoldID() != dwOfficialHoldID)
		return false; //not the pre-installed official hold
...
//True if activation should continue, false if not.
{
	...
	if (!stats.secretRooms.empty())
	{
		...
		pSecretsText = new CTextEffect(this, wStr.c_str(), F_FlashMessage, 7000);
		if (pSecretsText)
