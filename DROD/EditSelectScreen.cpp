// $Id: EditSelectScreen.cpp 10055 2012-03-31 05:06:14Z mrimer $
...
//Constructor.
{
	...
	static const int X_RENAME_LEVEL = X_CHOOSE_LEVEL_LABEL + 5;
	static const int Y_RENAME_LEVEL = Y_LEVEL_LBOX + CY_LEVEL_LBOX + CY_SPACE;
			...
			if (Key.keysym.mod & KMOD_CTRL)
			{
				...
				switch (pWidget->GetTagNo())
				{
					case TAG_LEVEL_LBOX:
					{
						...
						CDbLevel *pCutLevel = g_pTheDB->Levels.GetByID(
								this->pSelectedLevel->dwLevelID);
						delete this->pLevelCopy;
...
//Returns: whether a level was pasted
{
	...
	{
		//Don't allow cutting and pasting a level to the same hold.
		//This can be accomplished by dragging the level name to a new position.
		if (!bDifferentHold) return false;

		dwLevelID = this->pLevelCopy->dwLevelID;
		UINT dwNewEntranceID;
		const bool bPaste = SelectListID(this->pEntranceBox,
				this->pSelectedHold, dwNewEntranceID, MID_DestLevelPrompt) ==
				CEntranceSelectDialogWidget::OK;
		if (!bPaste) return false;
		SetCursor(CUR_Wait);
		if (!bDifferentHold)
		{
			this->pSelectedHold->RemoveLevel(dwLevelID,dwNewEntranceID);
			this->pSelectedHold->Update();
		} else {
			CDbHold *pHold = g_pTheDB->Holds.GetByID(this->pLevelCopy->dwHoldID);
			pHold->RemoveLevel(dwLevelID,dwNewEntranceID);
			pHold->Update();
			delete pHold;
		}
		...
		this->pLevelCopy->dwLevelIndex = this->pLevelCopy->dwOrderIndex = 0;
		this->pSelectedHold->InsertLevel(this->pLevelCopy,
				this->pLevelListBoxWidget->GetSelectedItem());
		this->pLevelCopy->Update();
		if (bDifferentHold)
			CDbLevels::UpdateExitIDs(this->pLevelCopy->dwLevelID);   //reset exits
		...
		CDbLevel *pNewLevel = this->pLevelCopy->MakeCopy(this->pSelectedHold, info);	//performs Update on hold and level
		if (!pNewLevel)
			return false;
		dwLevelID = pNewLevel->dwLevelID;

		//Insert level duplicate into current hold.
		pNewLevel->dwLevelIndex = pNewLevel->dwOrderIndex = 0;
		this->pSelectedHold->InsertLevel(pNewLevel);	//performs Update on hold and level
		if (bDifferentHold)
			CDbLevels::UpdateExitIDs(pNewLevel->dwLevelID);   //reset exits
