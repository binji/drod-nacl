// $Id: TileImageCalcs.cpp 9923 2012-03-08 23:17:24Z TFMurphy $
	...
	const UINT wO)
{
	switch (wType)
	{
		case M_BEETHRO_IN_DISGUISE: case M_CLONE: case M_GUNTHRO: //FIXME:Gunthro sword(?)
...
//other context.
{
	...
	if (IsValidMonsterType(wType))
	{
		...
		switch (wAnimFrame)
		{
			...
			case WADING_FRAME:
			{
				const UINT tile = GetWadingUnarmedEntityTile(wType, wO);
				if (tile)
					return tile;
			}
			//no break
		...
		if (wAnimFrame == SWORD_WADING_FRAME)
		{
			const UINT tile = GetWadingEntityTile(wType, wO);
			if (tile)
				return tile;
		}
		if (wAnimFrame == WADING_FRAME)
		{
			const UINT tile = GetWadingUnarmedEntityTile(wType, wO);
			if (tile)
				return tile;
...
//other context.
{
	static const UINT TileImageArray[TILE_COUNT] =
	{
		...
		CALC_NEEDED,      //T_FLOW_SOURCE
		CALC_NEEDED,      //T_FLOW_INNER
		CALC_NEEDED,      //T_FLOW_EDGE
...
//TI_* constant.
{
	...
	switch (wTileNo)
	{
		...
		break;
      case T_FLOW_INNER: case T_FLOW_EDGE: case T_FLOW_SOURCE:
...
//Params:
	const CDbRoom *pRoom,   //(in)   Room to use for calcs--not necessarily the
									//    current room.
	const UINT wCol, const UINT wRow,   //(in)   Square for which to calc.
	UINT& wX, UINT& wY,  //(out)  Position on pit side.
	UINT& wWidth)        //(out)  Width of this pit side to the right.
{
	ASSERT(pRoom->IsValidColRow(wCol, wRow));
	...
	wY = wLookRow == (UINT)(-1) ? wLookRow : wRow - wLookRow - 1;
...
//TI_* constant.
{
   ...
   const UINT wBriarIndex = pRoom->briars.getIndexAt(wCol, wRow);
	const bool bIsSource = pRoom->GetTSquare(wCol, wRow) == T_FLOW_SOURCE;
   ...
   //If north has flow, set bit 1.
	...
	if (wRow == 0)
		...
		if (wTTile == T_FLOW_SOURCE ||  //always connect to adjacent sources
   ...
   static const UINT TileImages[5][16] = {
	...
	{
		default:
		case T_FLOW_EDGE: wTypeIndex = 0 + pseudoRandomTileFrame(pRoom, wCol, wRow, 2); break;
		case T_FLOW_INNER:
		{
		...
		break;
		case T_FLOW_SOURCE: wTypeIndex = 4; break;
	}
...
#define IsDeepWaterTile(wTile) ((wTile) == T_WATER || (wTile) == T_TRAPDOOR2 || (wTile) == T_PLATFORM_W)
	...
	switch (wTileNo)
	{
		...
		case T_SHALLOW_WATER: case T_STEP_STONE: wTileType = 2; break;
