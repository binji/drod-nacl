// $Id: EditRoomWidget.cpp 10053 2012-03-31 05:04:44Z mrimer $
...
const
{
	...
	if (bSwordsmanAt)
	{
		...
		if (bIsTar(wTile) || wTile == T_ORB || wTile == T_BOMB ||
				bIsBriar(wTile) || wTile == T_STATION)
			return false;
	...
	switch (wSelectedObject)
	{
		...
		case T_WALL_H:
			...
			break;
		case T_WALL_M:
			if (bIsBriar(wTileNo[1]) || wTileNo[1] == T_MIRROR || wTileNo[1] == T_ORB)
				return false;
			if (wTileNo[2] != T_NOMONSTER && wTileNo[2] != M_SEEP &&
					!bIsMother(wTileNo[2]) && //tarstuff mothers allowed on walls
					wTileNo[2] != M_CHARACTER)
		...
		case T_MIRROR:
			...
			return !bSwordsmanAt &&
					(bIsFloor(wTileNo[0]) || (bIsWall(wTileNo[0]) && wTileNo[0] != T_WALL_M) ||
		...
		case T_CITIZEN:
		{
			...
			const bool bShallowWaterType =
					...
					wSelectedObject == T_ROCKGOLEM || wSelectedObject == T_GUARD ||
					wSelectedObject == T_STALWART2 || wSelectedObject == T_CLONE ||
					wSelectedObject == T_DECOY || wSelectedObject == T_MIMIC ||
					wSelectedObject == T_CITIZEN || wSelectedObject == T_SERPENT ||
		...
		case T_ROCKGIANT:
			...
			if (pMonster && pMonster->wType != M_ROCKGIANT)// && !bAllowSelf)
				return false;
		...
		case T_SWORDSMAN:
		{
			...
			if (!(bIsFloor(wTileNo[0]) || bIsTunnel(wTileNo[0]) ||
					...
					wTileNo[1] == T_BOMB || bIsBriar(wTileNo[1]) ||
					wTileNo[1] == T_OBSTACLE || wTileNo[1] == T_STATION ||
					pMonster)
				return false;
			return true;
	...
	const bool /*bMoveInProgress*/)
{
	...
	UINT wFrameIndex = this->pTileInfo[this->pRoom->ARRAYINDEX(pCharacter->wX, pCharacter->wY)].animFrame % 2;
	...
	if (draw_as_in_game)
	{
		...
		const UINT wTileImageNo = GetEntityTile(wIdentity, pCharacter->wLogicalIdentity, wO, wFrameIndex);
		...
		DrawTileImage(pCharacter->wX, pCharacter->wY, 0, 0,
				wTileImageNo, bDrawRaised, pDestSurface, true, !bAltPressed || pCharacter->IsVisible() ? 255 : 128);
	}
}
...
const
{
	...
	if (this->pHold)
	{
		HoldCharacter *pCustomChar = this->pHold->GetCharacter(wLogicalIdentity);
