// $Id: DrodScreen.cpp 10071 2012-04-02 23:24:39Z mrimer $
...
const char playSessions[] = "PlaySessions";
const char showCheckpoints[] = "ShowCheckpoints";
const char showSubtitlesWithVoices[] = "ShowSubtitlesWithVoices";
const char totalPlayTime[] = "TotalPlayTime";
const char TCBtotalPlayTime[] = "TCBTotalPlayTime";
...
const char groupEditorMenuItems[] = "GEMI";
const char moveCounterStr[] = "MoveCounter";
	...
	if (CueEvents.HasOccurred(CID_SwingSword))
	{
		...
		pRoomWidget->AddMLayerEffect(
				new CSwordSwingEffect(pRoomWidget,
					pGame->swordsman, static_cast<UINT>(*pOrientation)));
	}
		...
		while (pObj)
		{
			...
			if (pCoord->wValue >= M_OFFSET)
			{
				...
				if (bIsBriar(pCoord->wValue))
				{
					...
					const UINT tile = pRoomWidget->pwTSquareTI[index];
...
//*****************************************************************************
UINT CDrodScreen::GetEffectDuration(const CCurrentGame* pGame, const UINT baseDuration) const
...
//*****************************************************************************
UINT CDrodScreen::GetParticleSpeed(const CCurrentGame* pGame, const UINT baseSpeed) const
	...
	CCurrentGame* pGame) //[default=NULL]
{
	...
	const bool bHasEquipment = bHasSword;
	if (bHasEquipment)
	{
		wstr += wszCRLF;
...
//Sets the game to windowed mode and opens a browser with appropriate sell link.
{
	...
	if (!OpenExtBrowser("http://www.caravelgames.com/buyGatEBLinux.html"))
...
//Returns: whether the TCB hold is the full version (true), otherwise false
{
...
//*****************************************************************************
void CDrodScreen::ExportMusic()
//Exports music tracks from files on disk into a DROD-format media file.
{
	//!!hardcoded
	string strModName = "JtRH Music Pack";
	WSTRING modName;
	AsciiToUnicode(strModName.c_str(), modName);

	static const UINT TRACK_COUNT = 18;
	static char trackName[TRACK_COUNT][32] = {
		"After Paraguay","Ancient Machine I","Brood","Carve it Deep","Delver",
		"Devious","Far Enough","In My Skin","Ive Been Here","Larger View",
		"Last Gameshow","Leave Everything","MySmallBox","Sneaking around the docks",
		"The Reward","The Steady Smite","Uncovered","WithoutFear"
	};

	//Select file name.
	WSTRING wstrExportFile = modName;
	if (!ExportSelectFile(MID_SaveStylePath, wstrExportFile, EXT_DATA))
		return;

	//Temporarily import data objects.
	CIDSet ids;
	for (UINT wI=0; wI<TRACK_COUNT; ++wI)
	{
		WSTRING wstrFile;
		AsciiToUnicode(trackName[wI], wstrFile);
		PrepareDatumForExport(modName, wstrFile, ids, DATA_MUSIC);
	}

	//Gather .ini info.
	WSTRING iniInfo;
	AsciiToUnicode("[Songs]\n"
		"Deep SpacesAmbient=Carve it Deep.ogg" NEWLINE
		"Deep SpacesAttack=In My Skin.ogg;Far Enough.ogg" NEWLINE
		"Deep SpacesExit=Devious.ogg" NEWLINE
		"Deep SpacesPuzzle=Uncovered.ogg;Last Gameshow.ogg" NEWLINE
		"FoundationAmbient=Sneaking around the docks.ogg" NEWLINE
		"FoundationAttack=WithoutFear.ogg;Ancient Machine I.ogg" NEWLINE
		"FoundationExit=Delver.ogg" NEWLINE
		"FoundationPuzzle=After Paraguay.ogg;Brood.ogg" NEWLINE
		"IceworksAmbient=Leave Everything.ogg" NEWLINE
		"IceworksAttack=Ive Been Here.ogg;The Steady Smite.ogg" NEWLINE
		"IceworksExit=MySmallBox.ogg" NEWLINE
		"IceworksPuzzle=The Reward.ogg;Larger View.ogg" NEWLINE
		, iniInfo);
...
//Imports sound and graphics files from disk into the DB for DROD4:GatEB.
{
...
#define LOG_TEXT(wtext) {string strLog; UnicodeToAscii(wtext, strLog); strLog += NEWLINE; CStretchyBuffer text(strLog); f.WriteBufferToFile(wstrLogFilename.c_str(), text, true); }

	//If hold being exported is for demo, include selected styles only.
	//Otherwise include all of them.
	const bool bDemo = !IsGameFullVersion();
	if (bDemo)
	{
		//Export only Forest style.
		styles.clear();
		WSTRING demoStyle;
	...
	for (wStyleNo = 1; wStyleNo <= wStylesToExport; ++wStyleNo)
	{
		...
		//Import images.
		static const UINT NUM_GRAPHICS_FILES = 25;
		static const char graphicFilename[NUM_GRAPHICS_FILES][32] = {
			"Thanks1",
			"Background", "Bolts", "Clock1", "Clock2", "Credits", "Dialog",
			"Faces", "GameScreen", "GameScreenParts",
			"LevelStartBackground", "TitleLightMask",
			"Fog1", "Clouds1", "Sunshine1",
			"SignalBad", "SignalGood", "SignalNo", "SignalYes",
			"gunthro_title_back", "gunthro_title_front", "gunthro_title_windows", "geb_logo",
			"SellTexts", "StroutOfficeSShots"
		};
		for (wI=TEXTURE_COUNT + 1 + NUM_GRAPHICS_FILES; wI--; )
		{
			...
			if (wI >= TEXTURE_COUNT)
			{
				...
				if (wI == TEXTURE_COUNT)
				{
					...
					if (image_index == 0 && bDemo) {
						//Demo exit screen.  Import instead of full game exit screen.
						const WCHAR import_file[] = {{'S'},{'e'},{'l'},{'l'},{'1'},{0}};
						wstrImportFile = import_file;
					}
				}
			...
			{
				if (wI != DEEP_MOSAIC && wI != SHALLOW_MOSAIC) { //only present for some room styles
		...
		//Import music.
		//Include only the first style's music in the demo version.
		if (!bDemo || wStyleNo == 1) {
			WSTRING wstrDir;
			for (wI=SONG_MOOD_COUNT+5; wI--; )	//include other music too
			{
				...
				{
					//Include end game music for full hold only.
					...
					case SONG_MOOD_COUNT+3:
						if (wStyleNo == 2)
							g_pTheSound->GetSongFilepaths(SONGID_ENDOFTHEGAME, songlist);
					break;
	...
	const UINT dwPlayerID) //(in) Player
{
	...
	g_pTheBM->eyeCandy = pPlayer->Settings.GetVar("EyeCandy", BYTE(Metadata::GetInt(MetaKey::MAX_EYE_CANDY)));

	//Increment number of play sessions.
	pPlayer->Settings.SetVar(playSessions, pPlayer->Settings.GetVar(playSessions, UINT(0)) + 1);
	pPlayer->Update();
...
//Plays a video file on the current screen.  Uses the specified data record.
{
	...
	if (bIsCursorShowing)
		HideCursor();
	const bool bRes = CTheoraPlayer::playVideo(buffer, GetWidgetScreenSurface(), x, y);
	if (bIsCursorShowing)
	{
		ShowCursor();
		SetCursor();
